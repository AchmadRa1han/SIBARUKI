...
class CheckAsetDate extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        $total = $db->table('pertanahan_aset')->countAllResults();
        $nullCount = $db->table('pertanahan_aset')->where('tgl_terbit IS NULL')->countAllResults();
        $emptyCount = $db->table('pertanahan_aset')->where('tgl_terbit', '0000-00-00')->countAllResults();

        CLI::write("Total Aset: $total");
        CLI::write("Tanggal NULL: $nullCount");
        CLI::write("Tanggal 0000-00-00: $emptyCount");

        $sample = $db->table('pertanahan_aset')->where('tgl_terbit IS NOT NULL')->limit(3)->get()->getResultArray();
        CLI::write("
