...
<div class="space-y-6 pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-black text-blue-950 dark:text-white uppercase tracking-tight">Detail ARSINUM</h1>
                <p class="text-xs text-slate-500 font-bold uppercase tracking-widest">Tahun Anggaran <?= $item['tahun'] ?></p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="<?= base_url('arsinum/edit/'.$item['id']) ?>" class="px-5 py-2.5 bg-amber-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-amber-200 dark:shadow-none hover:bg-amber-600 transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden">
                <div class="absolute top-0 right-0 p-8 opacity-5">
                    <i data-lucide="droplets" class="w-32 h-32 text-blue-900 dark:text-white"></i>
                </div>
                
                <h3 class="text-sm font-black text-blue-950 dark:text-white uppercase tracking-[0.2em] mb-8 flex items-center gap-3">
                    <span class="w-8 h-1 bg-blue-600 rounded-full"></span> Rincian Pekerjaan
                </h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
                    <div class="md:col-span-2 space-y-1">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Jenis Pekerjaan</p>
                    ...
                    <div class="space-y-1">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Desa / Lokasi</p>
                        <p class="text-sm font-bold text-slate-800 dark:text-slate-100"><?= $item['desa'] ?></p>
                    </div>
                    <div class="space-y-1">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Volume</p>
                        <span class="inline-block px-3 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-600 text-[10px] font-black uppercase rounded-lg">
                            <?= $item['volume'] ?>
                        </span>
                    </div>
                    <div class="space-y-1">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tahun</p>
                        <p class="text-sm font-bold text-slate-800 dark:text-slate-100"><?= $item['tahun'] ?></p>
                    </div>
                </div>

                <div class="mt-12 pt-8 border-t border-slate-50 dark:border-slate-800 space-y-4">
                    <div class="space-y-1">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Pelaksana / Kontraktor</p>
                        <p class="text-sm font-medium text-slate-600 dark:text-slate-300 leading-relaxed"><?= $item['pelaksana'] ?: '-' ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-sm border border-slate-100 dark:border-slate-800">
                <h3 class="text-sm font-black text-blue-950 dark:text-white uppercase tracking-[0.2em] mb-6 flex items-center gap-3">
                    <span class="w-8 h-1 bg-emerald-500 rounded-full"></span> Anggaran & Pembiayaan
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Total Nilai Anggaran</p>
                        <p class="text-2xl font-black text-emerald-600">Rp <?= number_format($item['anggaran'], 0, ',', '.') ?></p>
                    </div>
                    <div class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Sumber Dana</p>
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 dark:border-slate-800 h-[400px] relative">
                ...
                <div class="absolute inset-0 z-[1001] bg-slate-900/10 backdrop-blur-[2px] flex items-center justify-center text-center p-8">
                    <div class="bg-white/90 dark:bg-slate-900/90 p-6 rounded-3xl shadow-2xl">
                        ...
                        <p class="text-[9px] font-black uppercase text-blue-950 dark:text-white">Koordinat Belum Ada</p>
                    </div>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="bg-blue-950 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
                <h4 class="text-[10px] font-black uppercase tracking-[0.3em] text-blue-400 mb-4">Sistem Log</h4>
                <div class="space-y-4">
                    <div class="flex justify-between items-center text-[10px]">
                        <span class="font-bold text-blue-300/60 uppercase">Data Masuk</span>
...
<script>
    ...
    function confirmDelete(id) {
        customConfirm('Hapus ARSINUM?', 'Hapus data kegiatan ini secara permanen?', 'danger').then(conf => {
