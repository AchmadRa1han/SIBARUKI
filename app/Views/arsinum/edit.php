...
<div class="max-w-7xl mx-auto space-y-6 pb-32 text-slate-900 dark:text-slate-200">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-blue-950 dark:text-white uppercase tracking-tighter">Edit ARSINUM</h1>
                <p class="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Perbarui Data Proyek Air Siap Minum</p>
            </div>
        </div>
        <div class="flex items-center gap-3 relative z-10">
            <a href="<?= base_url('arsinum/detail/' . $item['id']) ?>" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all active:scale-95">Batal</a>
        </div>
    </div>
    ...
    <form action="<?= base_url('arsinum/update/' . $item['id']) ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-300">
            <div class="p-6 border-b dark:border-slate-800 bg-blue-50/30 dark:bg-blue-950/30 flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-[11px] font-bold text-blue-900 dark:text-blue-400 uppercase tracking-[0.2em]">Data Teknis Aset</h3>
            ...
            <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="jenis_pekerjaan" value="<?= old('jenis_pekerjaan', $item['jenis_pekerjaan']) ?>" required class="w-full p-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 dark:text-slate-200 outline-none transition-all font-bold uppercase" placeholder="PEMBANGUNAN ARSINUM...">
                </div>
                <div>
                ...
                <div class="bg-blue-50 dark:bg-blue-950/30 p-6 rounded-2xl border border-blue-100 dark:border-blue-900/50">
                    ...
                    <input type="number" name="anggaran" value="<?= old('anggaran', $item['anggaran']) ?>" required class="w-full bg-transparent border-none text-xl font-bold text-blue-950 dark:text-white p-0 focus:ring-0 outline-none">
                </div>
                <div>
                ...
                <div class="md:col-span-1">
                    ...
                    <div class="relative group">
                        ...
                        <div id="foto_before_preview" class="w-full h-32 bg-slate-50 dark:bg-slate-950 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center overflow-hidden transition-all group-hover:border-amber-500 group-hover:bg-amber-50/5">
                            <?php if (!empty($item['foto_before'])): ?>
                                <img src="<?= base_url('uploads/arsinum/' . $item['foto_before']) ?>" class="w-full h-full object-cover">
                            <?php else: ?>
                                <i data-lucide="image-plus" class="w-6 h-6 text-slate-300 mb-1.5"></i>
                                <span class="text-[7px] font-bold text-slate-400 uppercase tracking-widest">Unggah Foto Before Baru</span>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                <div class="md:col-span-1">
                    ...
                    <div class="relative group">
                        ...
                        <div id="foto_after_preview" class="w-full h-32 bg-slate-50 dark:bg-slate-950 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center overflow-hidden transition-all group-hover:border-amber-500 group-hover:bg-amber-50/5">
                            <?php if (!empty($item['foto_after'])): ?>
                                <img src="<?= base_url('uploads/arsinum/' . $item['foto_after']) ?>" class="w-full h-full object-cover">
                            <?php else: ?>
                                <i data-lucide="image-plus" class="w-6 h-6 text-slate-300 mb-1.5"></i>
                                <span class="text-[7px] font-bold text-slate-400 uppercase tracking-widest">Unggah Foto After Baru</span>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
