...
<div class="max-w-7xl mx-auto space-y-8 pb-32">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center gap-6">
            ...
            <div>
                <h1 class="text-3xl md:text-4xl font-black text-blue-950 dark:text-white uppercase tracking-tighter">Edit ARSINUM</h1>
                <p class="text-sm text-slate-500 font-bold uppercase tracking-widest mt-1">Perbarui Data Proyek Air Siap Minum</p>
            </div>
        </div>
        <div class="flex items-center gap-4 relative z-10">
            <a href="<?= base_url('arsinum/detail/' . $item['id']) ?>" class="px-8 py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all active:scale-95">Batal</a>
        </div>
    </div>
    ...
    <form action="<?= base_url('arsinum/update/' . $item['id']) ?>" method="post">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-300">
            <div class="p-8 border-b dark:border-slate-800 bg-blue-50/30 dark:bg-blue-950/30 flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-xs font-black text-blue-900 dark:text-blue-400 uppercase tracking-[0.2em]">Data Teknis Aset</h3>
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Sinkronisasi Informasi & Wilayah</p>
                </div>
            </div>
            <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="jenis_pekerjaan" value="<?= old('jenis_pekerjaan', $item['jenis_pekerjaan']) ?>" required class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 dark:text-slate-200 outline-none transition-all font-black uppercase" placeholder="PEMBANGUNAN ARSINUM...">
                </div>
                <div>
                    ...
                    <input type="text" name="volume" value="<?= old('volume', $item['volume']) ?>" required class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 dark:text-slate-200 outline-none transition-all font-bold uppercase">
                </div>
                <div>
                ...
                <div class="bg-blue-50 dark:bg-blue-950/30 p-8 rounded-[2rem] border border-blue-100 dark:border-blue-900/50">
                    ...
                    <input type="number" name="anggaran" value="<?= old('anggaran', $item['anggaran']) ?>" required class="w-full bg-transparent border-none text-2xl font-black text-blue-950 dark:text-white p-0 focus:ring-0 outline-none">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="desa" value="<?= old('desa', $item['desa']) ?>" required class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 dark:text-slate-200 outline-none transition-all font-bold uppercase">
                </div>
                <div class="md:col-span-2">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-6 pt-8">
            <div class="flex items-center gap-4 text-slate-400">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest leading-relaxed max-w-md">Metadata perubahan akan dicatat untuk audit infrastruktur.</p>
            </div>
