...
<div class="space-y-8 pb-12">
    ...
    <nav class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 no-print">
        ...
        <span class="text-blue-600">ARSINUM</span>
    </nav>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h1 class="text-3xl md:text-4xl font-black text-blue-950 dark:text-white uppercase tracking-tighter">ARSINUM</h1>
            <p class="text-slate-500 dark:text-slate-400 font-medium text-sm mt-1">Pemetaan Teknologi Air Siap Minum Kabupaten Sinjai.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3 relative z-10">
            ...
            <a href="<?= base_url('arsinum/export-excel') ?>" class="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50 hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center gap-3 shadow-sm">
                <i data-lucide="download" class="w-4 h-4"></i> Export Excel
            </a>
            <?php if (has_permission('create_rtlh')): ?>
            <a href="<?= base_url('arsinum/create') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-blue-600/20 transition-all active:scale-95 flex items-center gap-3 group">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-6">
            <div class="flex items-center gap-4 w-full lg:w-auto">
                <div class="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl w-full md:w-auto">
                    <button class="px-6 py-2.5 bg-white dark:bg-slate-700 text-blue-600 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">Daftar Pekerjaan</button>
                </div>
            </div>

            <form action="<?= base_url('arsinum') ?>" method="get" class="flex flex-col md:flex-row items-center gap-3 w-full lg:w-auto" id="filter-form">
                ...
                <div class="relative w-full md:w-64">
                    <input type="text" name="search" value="<?= $search ?>" placeholder="Cari pekerjaan..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-3 pl-11 focus:ring-2 focus:ring-blue-500 transition-all">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div id="bulk-action-bar" class="absolute top-0 left-0 right-0 z-50 bg-blue-950 text-white p-5 transform -translate-y-full transition-transform duration-500 flex items-center justify-between px-10">
            <div class="flex items-center gap-5">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest opacity-70 hidden md:block">Aksi massal untuk data arsinum</p>
            </div>
        ...
        <div class="p-8 border-b border-slate-50 dark:border-slate-800">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-lg font-black text-blue-950 dark:text-white uppercase tracking-tight">Daftar Aset Arsinum</h3>
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Manajemen Infrastruktur & Teknologi</p>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-4 py-5 w-64 cursor-pointer hover:text-blue-600 transition-colors" onclick="applySort('jenis_pekerjaan')">
                            <div class="flex items-center gap-2">
                                Jenis Pekerjaan
                                <i data-lucide="arrow-up-down" class="w-3 h-3 opacity-30"></i>
                            </div>
                        </th>
                        <th class="px-4 py-5 w-32 text-center">Volume</th>
                        <th class="px-4 py-5 w-56">Wilayah Administratif</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[11px]">
                    <?php if (!empty($arsinum)): foreach($arsinum as $item): ?>
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-300">
                        ...
                        <td class="px-4 py-4 text-center">
                            <span class="px-4 py-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl font-black text-[10px] border border-blue-100 dark:border-blue-800 shadow-sm uppercase">
                                <?= $item['volume'] ?>
                            </span>
                        </td>
                        <td class="px-4 py-4">
                            <div class="flex flex-col gap-0.5">
                                <span class="font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight"><?= $item['desa'] ?></span>
                                <span class="text-[9px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1">
                                    <i data-lucide="map-pin" class="w-2.5 h-2.5"></i> Kec. <?= $item['kecamatan'] ?>
                                </span>
                            </div>
                        </td>
                        <td class="px-8 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <?php endif; ?>
                                <a href="<?= base_url('arsinum/detail/'.$item['id']) ?>" class="p-2.5 bg-blue-950 dark:bg-blue-600 text-white rounded-xl shadow-md hover:scale-110 transition-all active:scale-95" title="Detail Data"><i data-lucide="eye" class="w-4 h-4"></i></a>
...
<script>
    ...
    function confirmDelete(id) {
        customConfirm('Hapus Data?', 'Apakah Anda yakin ingin menghapus data ARSINUM ini? Tindakan ini tidak dapat dibatalkan.', 'danger').then(conf => {
    ...
    async function handleBulkDelete() {
        ...
        if (ok) {
            ...
            try {
                const response = await fetch('<?= base_url('arsinum/bulk-delete') ?>', {
                    method: 'POST',
                    body: formData,
