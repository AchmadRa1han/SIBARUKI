...
<div class="max-w-7xl mx-auto space-y-6 pb-12 text-slate-900 dark:text-slate-200">
    ...
    <!-- Header -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        <div class="absolute top-0 right-0 w-48 h-48 bg-blue-600/5 rounded-full -mr-24 -mt-24 blur-3xl"></div>
        <div class="relative z-10">
            <h1 class="text-2xl md:text-3xl font-bold text-blue-950 dark:text-white uppercase tracking-tighter">Rekapitulasi Desa</h1>
        ...
        <div class="flex items-center gap-3 relative z-10">
            ...
            <a href="<?= base_url('rtlh') ?>" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all active:scale-95 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        ?>
        <div class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:shadow-md group relative overflow-hidden">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">Target RTLH</span>
            </div>
            <h3 class="text-4xl font-bold text-blue-950 dark:text-white tracking-tighter mb-1 relative z-10"><?= number_format($totalRtlh) ?></h3>
        ...
        <div class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:shadow-md group relative overflow-hidden">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">Capaian RLH</span>
            </div>
            <h3 class="text-4xl font-bold text-emerald-600 tracking-tighter mb-1 relative z-10"><?= number_format($totalRlh) ?></h3>
            <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest relative z-10">Unit Berhasil Ditransformasi</p>
        </div>

        <div class="bg-blue-950 p-8 rounded-2xl shadow-xl shadow-blue-950/20 transition-all hover:-translate-y-1 group relative overflow-hidden">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-bold text-blue-300/40 uppercase tracking-[0.2em]">Total Data</span>
            </div>
            <h3 class="text-4xl font-bold text-white tracking-tighter mb-1 relative z-10"><?= number_format($totalSemua) ?></h3>
            <p class="text-[9px] text-blue-300/60 font-bold uppercase tracking-widest relative z-10">Total Database Terverifikasi</p>
        </div>
    </div>

    <!-- Search Bar -->
    <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between transition-all duration-300">
        <form action="<?= base_url('rtlh/rekap-desa') ?>" method="GET" class="w-full md:w-96 relative group">
            ...
            <input type="text" name="keyword" value="<?= esc($keyword ?? '') ?>" placeholder="Cari nama desa..." 
                class="w-full pl-11 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-bold focus:ring-2 focus:ring-blue-500/20 transition-all outline-none">
        </form>
        <?php if ($keyword ?? false): ?>
            <a href="<?= base_url('rtlh/rekap-desa') ?>" class="text-[10px] font-bold text-rose-500 uppercase tracking-widest hover:underline flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        <div class="p-6 border-b border-slate-50 dark:border-slate-800 flex items-center gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-blue-950 dark:text-white uppercase tracking-tight">Sebaran Data per Desa</h3>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[10px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-200">
                        ...
                        <td class="px-6 py-4">
                            <span class="font-bold text-blue-950 dark:text-white uppercase tracking-wider text-xs"><?= $item['desa'] ?: 'N/A' ?></span>
                        </td>
                        <td class="px-6 py-4 text-center font-mono font-bold text-slate-500"><?= $item['desa_id'] ?: '-' ?></td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-block px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg font-bold text-xs border border-blue-100 dark:border-blue-800 shadow-sm">
                                <?= $item['total_rtlh'] ?>
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-block px-3 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-lg font-bold text-xs border border-emerald-100 dark:border-emerald-800 shadow-sm">
                                <?= $item['total_rlh'] ?>
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="font-bold text-blue-950 dark:text-white text-base"><?= $item['total_semua'] ?></span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <a href="<?= base_url('rtlh?keyword=' . urlencode($item['desa']) . '&status=semua') ?>" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-950 dark:bg-blue-600 text-white rounded-xl text-[9px] font-bold uppercase tracking-widest hover:scale-105 transition-all active:scale-95 shadow-lg shadow-blue-900/20">
