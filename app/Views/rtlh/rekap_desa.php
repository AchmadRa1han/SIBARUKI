...
<div class="space-y-6 pb-12">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <?php 
            $totalUnit = array_sum(array_column($rekap, 'total'));
        ?>
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:shadow-md group">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Keseluruhan</span>
            </div>
            <h3 class="text-4xl font-black text-blue-950 dark:text-white"><?= number_format($totalUnit) ?></h3>
            <p class="text-[10px] text-slate-400 font-bold uppercase mt-1">Unit RTLH Terverifikasi</p>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[9px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-8 py-4 text-center w-48">Jumlah Unit RTLH</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[10px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-200">
                        ...
                        <td class="px-8 py-4 text-center">
                            <span class="inline-block px-4 py-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl font-black text-sm shadow-sm border border-blue-100 dark:border-blue-800">
                                <?= $item['total'] ?> <span class="text-[10px] ml-1 opacity-60">Unit</span>
                            </span>
                        </td>
                        <td class="px-8 py-4 text-center">
                            <a href="<?= base_url('rtlh?keyword=' . urlencode($item['desa'])) ?>" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-950 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-black transition-all active:scale-95 shadow-lg shadow-blue-900/20">
                                <i data-lucide="list" class="w-3.5 h-3.5"></i> Lihat Detail
                            </a>
                        </td>
                    </tr>
                    <?php endforeach; else: ?>
                        <tr><td colspan="5" class="px-8 py-12 text-center text-slate-400 font-bold uppercase text-[10px]">Data tidak tersedia</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
