...
<div id="report-content" class="max-w-7xl mx-auto space-y-6 pb-24 text-slate-900 dark:text-slate-200">
    ...
    <!-- MAIN HEADER -->
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm no-print relative overflow-hidden">
        ...
        <div class="flex flex-col md:flex-row md:items-center gap-6 relative z-10">
            ...
            <div>
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    <span class="px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full text-[9px] font-black uppercase tracking-widest border border-blue-100 dark:border-blue-900/50">
                        SRV-<?= str_pad($rumah['id_survei'] ?? '0', 5, '0', STR_PAD_LEFT) ?>
                    </span>
                    <?php if (($rumah['status_bantuan'] ?? 'Belum Menerima') == 'Belum Menerima') : ?>
                        <span class="px-3 py-1 bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 rounded-full text-[9px] font-black uppercase tracking-widest border border-rose-100 dark:border-rose-900/50">TARGET (RTLH)</span>
                    <?php else: ?>
                        <span class="px-3 py-1 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-full text-[9px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-900/50">TUNTAS (RLH) - <?= $rumah['tahun_bansos'] ?></span>
                    <?php endif; ?>
                </div>
                <h1 class="text-2xl md:text-3xl font-black text-blue-950 dark:text-white tracking-tighter uppercase mb-1"><?= $penerima['nama_kepala_keluarga'] ?? '-' ?></h1>
        ...
        <div class="flex flex-wrap items-center gap-2 relative z-10">
            ...
            <?php if (has_permission('edit_rtlh')) : ?>
                <?php if (($rumah['status_bantuan'] ?? 'Belum Menerima') == 'Belum Menerima') : ?>
                <button onclick="openModalTuntas()" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-600/10 transition-all active:scale-95 flex items-center gap-2">
                    <i data-lucide="check-circle" class="w-4 h-4"></i> Tandai Tuntas
                </button>
                <?php endif; ?>
                <a href="<?= base_url('rtlh/edit/' . ($rumah['id_survei'] ?? '')) ?>" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-600/10 transition-all active:scale-95 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-300">
                <div class="p-6 border-b dark:border-slate-800 flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-[11px] font-black text-blue-950 dark:text-white uppercase tracking-[0.2em]">Identitas Pemilik</h3>
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-10">
                    ...
                    <div>
                        ...
                        <span class="px-2.5 py-1 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 rounded-lg text-xs font-black border border-emerald-100 dark:border-emerald-900"><?= $penerima['penghasilan_per_bulan'] ?? '-' ?></span>
                    </div>
                    <div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-300">
                <div class="p-6 border-b dark:border-slate-800 flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-[11px] font-black text-blue-950 dark:text-white uppercase tracking-[0.2em]">Lokasi & Aset Lahan</h3>
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-10">
                    ...
                    <div class="bg-blue-600 p-5 rounded-[1.5rem] text-white shadow-lg shadow-blue-600/10">
                        <p class="text-[8px] font-black text-blue-100 uppercase mb-1 tracking-[0.2em]">Luas Rumah</p>
                        <p class="text-2xl font-black italic"><?= $rumah['luas_rumah_m2'] ?? '0' ?><span class="text-xs font-bold ml-1 opacity-60">m²</span></p>
                    </div>
                    <div class="bg-slate-900 dark:bg-slate-800 p-5 rounded-[1.5rem] text-white shadow-lg">
                        <p class="text-[8px] font-black text-slate-400 uppercase mb-1 tracking-[0.2em]">Luas Lahan</p>
                        <p class="text-2xl font-black italic"><?= $rumah['luas_lahan_m2'] ?? '0' ?><span class="text-xs font-bold ml-1 opacity-60">m²</span></p>
                    </div>
                    <div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-300 no-print">
                <div class="p-6 border-b dark:border-slate-800 flex items-center gap-3">
                    <div class="w-9 h-9 bg-slate-100 dark:bg-slate-800 rounded-lg flex items-center justify-center text-blue-600">
                        <i data-lucide="camera" class="w-4.5 h-4.5"></i>
                    </div>
                    <div>
                        <h3 class="text-[11px] font-black text-blue-950 dark:text-white uppercase tracking-[0.2em]">Galeri Dokumentasi</h3>
                    ...
                    <?php 
                        ...
                        foreach($fotos as $f_key => $f_label):
                            $photoPath = !empty($rumah[$f_key]) && file_exists(FCPATH . 'uploads/rtlh/' . $rumah[$f_key]) 
                                ...
                                : null;
                    ?>
                    <div class="group relative bg-slate-50 dark:bg-slate-950 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-800 aspect-video">
                        <?php if($photoPath): ?>
                            ...
                            <div class="absolute inset-0 bg-blue-950/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center p-4 text-center">
                                <p class="text-white font-black uppercase tracking-widest text-[10px] mb-3"><?= $f_label ?></p>
                                <button onclick="window.open('<?= $photoPath ?>', '_blank')" class="px-4 py-2 bg-white text-blue-950 rounded-lg text-[9px] font-black uppercase tracking-widest hover:scale-105 transition-all">Zoom</button>
                            </div>
                        <?php else: ?>
                            <div class="w-full h-full flex flex-col items-center justify-center text-slate-300 dark:text-slate-800">
                                ...
                                <span class="text-[9px] font-black uppercase tracking-widest opacity-40">Belum Ada Foto</span>
                            </div>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                <div class="p-6 border-b dark:border-slate-800 bg-blue-600 text-white">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-[11px] font-black uppercase tracking-[0.2em]">Penilaian Teknis</h3>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 space-y-2">
                                <div class="flex items-center justify-between">
                                    <span class="text-[10px] font-black text-blue-900 dark:text-blue-400 uppercase tracking-widest"><?= $m['label'] ?></span>
                                    <span class="text-[9px] font-bold text-slate-400 italic"><?= $m['mat'] ?></span>
                                </div>
                                <div class="w-full text-center py-1.5 rounded-lg text-[8px] font-black uppercase border <?= getStatusBadge($m['st']) ?>">
                                    <?= $m['st'] ?>
                                </div>
                            </div>
                            <?php endforeach; ?>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800">
                                <p class="text-[7px] font-black text-slate-400 uppercase mb-1">Air Minum</p>
                                <p class="text-[9px] font-black text-slate-700 dark:text-white uppercase leading-tight"><?= $rumah['sumber_air_minum'] ?? '-' ?></p>
                            </div>
                            <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800">
                                <p class="text-[7px] font-black text-slate-400 uppercase mb-1">Penerangan</p>
                                <p class="text-[9px] font-black text-slate-700 dark:text-white uppercase leading-tight"><?= $rumah['sumber_penerangan'] ?? '-' ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Meta Info -->
            <div class="bg-blue-950 rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden group">
                <div class="absolute -right-4 -bottom-4 opacity-10 group-hover:scale-110 transition-transform duration-700">
                    <i data-lucide="info" class="w-32 h-32"></i>
                </div>
                <h4 class="text-[9px] font-black uppercase tracking-[0.3em] text-blue-400 mb-4 flex items-center gap-2">
                    Informasi Sistem
                </h4>
                <div class="space-y-4 relative z-10">
                    <div class="flex justify-between items-center text-[9px]">
                        <span class="font-bold text-blue-300/60 uppercase tracking-widest">Dibuat</span>
                        <span class="font-black text-blue-50"><?= !empty($rumah['created_at']) ? date('d/m/y H:i', strtotime($rumah['created_at'])) : '-' ?></span>
                    </div>
                    <div class="flex justify-between items-center text-[9px]">
                        <span class="font-bold text-blue-300/60 uppercase tracking-widest">Update</span>
                        <span class="font-black text-blue-50"><?= !empty($rumah['updated_at']) ? date('d/m/y H:i', strtotime($rumah['updated_at'])) : '-' ?></span>
                    </div>
                </div>
            </div>
        </div>
...
<div id="modal-tuntas" class="fixed inset-0 z-[10001] flex items-center justify-center p-4 hidden">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-8 max-w-md w-full relative z-10 shadow-2xl border border-slate-100 dark:border-slate-800 transition-all">
        <div class="flex flex-col items-center text-center mb-6">
            <div class="w-16 h-16 rounded-2xl mb-4 flex items-center justify-center bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 shadow-inner">
                <i data-lucide="award" class="w-8 h-8"></i>
            </div>
            <h3 class="text-xl font-black text-blue-950 dark:text-white uppercase tracking-tight mb-1">Tuntas Bantuan</h3>
        ...
        <form action="<?= base_url('rtlh/mark-tuntas/' . $rumah['id_survei']) ?>" method="POST" class="space-y-4">
            <?= csrf_field() ?>
            <div>
                <label class="block text-[9px] font-black text-slate-400 uppercase mb-2 tracking-widest ml-1">Tahun Anggaran</label>
                <input type="number" name="tahun_bansos" value="<?= date('Y') ?>" min="2000" max="2099" class="w-full p-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 dark:text-white outline-none transition-all font-bold" required>
            </div>
            <div>
                <label class="block text-[9px] font-black text-slate-400 uppercase mb-2 tracking-widest ml-1">Nama Program</label>
                <input type="text" name="program_bansos" placeholder="Contoh: BSPS, APBD Kab..." class="w-full p-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 dark:text-white outline-none transition-all font-bold" required>
            </div>
