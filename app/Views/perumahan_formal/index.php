...
<div class="space-y-8 pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h1 class="text-3xl md:text-4xl font-black text-blue-950 dark:text-white uppercase tracking-tighter">Perumahan Formal</h1>
            <p class="text-slate-500 dark:text-slate-400 font-medium text-sm mt-1">Pemetaan dan Monitoring Sebaran Perumahan Formal Kabupaten Sinjai.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3 relative z-10">
            <div class="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-blue-100 dark:border-blue-800/50 shadow-sm">
                <?= number_format($total_perumahan ?? 0) ?> Lokasi Terdata
            </div>
            <a href="<?= base_url('perumahan-formal/export-excel') ?>" class="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50 hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center gap-3 shadow-sm">
                <i data-lucide="download" class="w-4 h-4"></i> Export Excel
            </a>
            <?php if (has_permission('create_rtlh')): ?>
            <a href="<?= base_url('perumahan-formal/create') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-blue-600/20 transition-all active:scale-95 flex items-center gap-3 group">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-6">
            <div class="flex items-center gap-4 w-full lg:w-auto">
                <div class="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl w-full md:w-auto">
                    <button class="px-6 py-2.5 bg-white dark:bg-slate-700 text-blue-600 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">Daftar Perumahan</button>
                </div>
            </div>

            <form action="<?= base_url('perumahan-formal') ?>" method="get" class="flex flex-col md:flex-row items-center gap-3 w-full lg:w-auto" id="filter-form">
                ...
                <div class="relative w-full md:w-72">
                    <input type="text" name="keyword" value="<?= $keyword ?? '' ?>" placeholder="Cari nama/pengembang..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-3 pl-11 focus:ring-2 focus:ring-blue-500 transition-all">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div id="bulk-action-bar" class="absolute top-0 left-0 right-0 z-50 bg-blue-950 text-white p-5 transform -translate-y-full transition-transform duration-500 flex items-center justify-between px-10">
            <div class="flex items-center gap-5">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest opacity-70 hidden md:block">Aksi massal untuk data perumahan</p>
            </div>
        ...
        <div class="p-8 border-b border-slate-50 dark:border-slate-800">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-lg font-black text-blue-950 dark:text-white uppercase tracking-tight">Data Induk Perumahan</h3>
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Monitoring Sektor Perumahan Formal</p>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-4 py-5 w-64">Nama Perumahan</th>
                        <th class="px-4 py-5 w-48">Pengembang / Developer</th>
                        <th class="px-4 py-5 w-32 text-center">Tahun</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[11px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-300">
                        ...
                        <td class="px-4 py-4">
                            <span class="font-black text-blue-950 dark:text-white uppercase truncate block text-sm mb-0.5"><?= $item['nama_perumahan'] ?></span>
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Kawasan Permukiman Formal</span>
                        </td>
                        <td class="px-4 py-4">
                            <span class="font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight"><?= $item['pengembang'] ?></span>
                        </td>
                        <td class="px-4 py-4 text-center">
                            <span class="px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg font-black text-[10px]">
                        ...
                        <td class="px-8 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <button onclick="focusMap(<?= $item['latitude'] ?>, <?= $item['longitude'] ?>)" class="p-2.5 bg-white dark:bg-slate-800 text-blue-600 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 hover:bg-blue-600 hover:text-white transition-all active:scale-95" title="Lihat di Peta"><i data-lucide="map-pin" class="w-4 h-4"></i></button>
                                <?php endif; ?>
                                <a href="<?= base_url('perumahan-formal/detail/'.$item['id']) ?>" class="p-2.5 bg-blue-950 dark:bg-blue-600 text-white rounded-xl shadow-md hover:scale-110 transition-all active:scale-95" title="Detail Data"><i data-lucide="eye" class="w-4 h-4"></i></a>
                                <?php if (has_permission('delete_rtlh')): ?>
                                <button onclick="confirmDelete(<?= $item['id'] ?>, '<?= addslashes($item['nama_perumahan']) ?>')" class="p-2.5 bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 rounded-xl hover:bg-rose-600 hover:text-white transition-all active:scale-95" title="Hapus Data"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                                <?php endif; ?>
                            </div>
                        </td>
                    </tr>
...
<script>
        ...
        try {
            ...
            perumahanData.forEach(item => {
                if (item.latitude && item.longitude) {
                    ...
                    const marker = L.circleMarker(coords, { radius: 10, fillColor: "#1e1b4b", color: "#fff", weight: 2, fillOpacity: 0.8 });
                    marker.bindPopup(`
                        <div class="bg-blue-950 text-white p-4 rounded-t-xl"><p class="text-[8px] font-black uppercase tracking-[0.2em] text-blue-400 mb-1">Perumahan Formal</p><h5 class="text-xs font-black uppercase leading-tight">${item.nama_perumahan}</h5></div>
                        <div class="p-4 bg-white dark:bg-slate-900 space-y-3 rounded-b-xl"><p class="text-[10px] font-bold text-slate-700 dark:text-slate-300">🏢 ${item.pengembang}</p><p class="text-[10px] font-bold text-slate-700 dark:text-slate-300">📏 ${parseFloat(item.luas_kawasan_ha).toFixed(2)} Ha</p><a href="<?= base_url('perumahan-formal/detail/') ?>/${item.id}" class="block w-full py-2 bg-blue-950 text-white text-center text-[9px] font-black uppercase tracking-widest rounded-xl transition-all">Detail</a></div>
                    `);
    ...
    async function handleBulkDelete() {
        ...
        const ok = await window.customConfirm('Hapus Massal?', `Hapus ${ids.length} data perumahan yang dipilih?`, 'danger');
        if (ok) {
