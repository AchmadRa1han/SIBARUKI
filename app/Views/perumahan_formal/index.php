            ...
            perumahanData.forEach(item => {
                if (item.latitude && item.longitude) {
                    ...
                    marker.bindPopup(`
                        ...
                        <div class="p-3 bg-white dark:bg-slate-900 space-y-2 rounded-b-xl"><p class="text-[9px] font-bold text-slate-700 dark:text-slate-300">🏢 ${item.pengembang}</p><a href="<?= base_url('perumahan-formal/detail/') ?>/${item.id}" class="block w-full py-2 bg-blue-600 hover:bg-blue-700 text-white text-center text-[10px] font-black uppercase tracking-[0.1em] rounded-xl shadow-lg transition-all">Detail</a></div>
                    `);
