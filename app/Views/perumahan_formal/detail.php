...
<div class="max-w-7xl mx-auto space-y-6 pb-12 text-slate-900 dark:text-slate-200">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center gap-4">
            ...
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-blue-950 dark:text-white uppercase tracking-tighter leading-tight"><?= $item['nama_perumahan'] ?></h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm group relative overflow-hidden transition-all hover:shadow-xl">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">Luas Kawasan</span>
            </div>
            <h3 class="text-4xl font-bold text-blue-950 dark:text-white tracking-tighter mb-1 relative z-10"><?= number_format($item['luas_kawasan_ha'], 2) ?><span class="text-lg ml-1 text-slate-400">Ha</span></h3>
        ...
        <div class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm group relative overflow-hidden transition-all hover:shadow-xl">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">Tahun Bangun</span>
            </div>
            <h3 class="text-4xl font-bold text-emerald-600 tracking-tighter mb-1 relative z-10"><?= $item['tahun_pembangunan'] ?></h3>
        ...
        <div class="bg-blue-950 p-8 rounded-2xl shadow-xl shadow-blue-950/20 relative overflow-hidden transition-all hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-bold text-blue-300/40 uppercase tracking-[0.2em]">Status Spasial</span>
            </div>
            <h3 class="text-xl font-bold text-white tracking-tight mb-1 relative z-10 uppercase"><?= $item['wkt'] ? 'Polygon Map' : 'Point Only' ?></h3>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-800 relative group aspect-video lg:aspect-auto lg:h-[600px]">
                ...
                <div class="absolute top-6 left-6 z-[1000] flex flex-col gap-2">
                    <div class="bg-blue-950/90 backdrop-blur-xl text-white px-4 py-2 rounded-xl shadow-2xl border border-white/10 flex items-center gap-3">
                        ...
                        <span class="text-[9px] font-bold uppercase tracking-[0.2em]">Live Map Preview</span>
                    </div>
                </div>
        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden transition-all duration-300">
                ...
                <div class="space-y-8 relative z-10">
                    <div>
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Nama Perumahan</p>
                        <p class="text-sm font-bold text-blue-950 dark:text-white uppercase leading-relaxed"><?= $item['nama_perumahan'] ?></p>
                    </div>

                    <div>
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Pengembang / Developer</p>
                        <p class="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase leading-relaxed"><?= $item['pengembang'] ?></p>
                    </div>

                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Latitude</p>
                            <p class="text-xs font-mono font-bold text-slate-700 dark:text-slate-300"><?= $item['latitude'] ?></p>
                        </div>
                        <div>
                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Longitude</p>
                    ...
                    <div class="pt-6 border-t border-slate-50 dark:border-slate-800">
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-3">Metode Pemetaan</p>
                        <div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-[10px] font-bold text-blue-950 dark:text-white uppercase tracking-tight"><?= $item['wkt'] ? 'POLYGON (WKT)' : 'POINT DATA' ?></p>
            ...
            <div class="bg-slate-900 rounded-2xl p-8 text-white shadow-2xl relative overflow-hidden group">
                ...
                <div class="space-y-4 relative z-10">
                    <div class="flex justify-between items-center text-[9px]">
                        ...
                        <span class="font-bold text-slate-300 tracking-wider"><?= date('d/m/y H:i', strtotime($item['created_at'])) ?></span>
                    </div>
                    <div class="flex justify-between items-center text-[9px]">
                        <span class="font-bold text-slate-500 uppercase tracking-widest">Last Update</span>
                        <span class="font-bold text-blue-400 tracking-wider"><?= date('d/m/y H:i', strtotime($item['updated_at'])) ?></span>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function initMap() {
        ...
        map = L.map('map', { layers: [googleSat], zoomControl: false }).setView(coords, 17);
