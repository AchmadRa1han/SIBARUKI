...
<div class="max-w-7xl mx-auto space-y-6 pb-12 text-slate-900 dark:text-slate-200">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="flex items-center gap-2 relative z-10">
            ...
            <a href="<?= base_url('psu/edit/' . $jalan['id']) ?>" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-amber-500/20 transition-all active:scale-95 flex items-center gap-2">
                <i data-lucide="edit-3" class="w-4 h-4"></i> Edit
            </a>
            <?php endif; ?>
            <a href="<?= base_url('psu') ?>" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all active:scale-95">Kembali</a>
        </div>
    </div>

    <!-- Ringkasan Statistik -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm group relative overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Nilai Aset</span>
            </div>
            <h3 class="text-4xl font-black text-indigo-600 tracking-tighter mb-1 relative z-10"><?= number_format($jalan['jalan'], 2) ?></h3>
        ...
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm group relative overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Tipe Struktur</span>
            </div>
            <h3 class="text-xl font-black text-blue-950 dark:text-white tracking-tight mb-1 relative z-10 uppercase">LineString</h3>
        ...
        <div class="bg-blue-950 p-8 rounded-[2rem] shadow-xl shadow-blue-950/20 relative overflow-hidden transition-all hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <span class="text-[9px] font-black text-blue-300/40 uppercase tracking-[0.2em]">Sync Date</span>
            </div>
            <h3 class="text-xl font-black text-white tracking-tight mb-1 relative z-10"><?= date('d M Y', strtotime($jalan['updated_at'])) ?></h3>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 dark:border-slate-800 relative group aspect-video lg:aspect-auto lg:h-[600px]">
                ...
                <div class="absolute top-6 left-6 z-[1000] flex flex-col gap-2">
                    <div class="bg-blue-950/90 backdrop-blur-xl text-white px-4 py-2 rounded-xl shadow-2xl border border-white/10 flex items-center gap-3">
                        ...
                        <span class="text-[9px] font-black uppercase tracking-[0.2em]">Visualisasi Geospasial</span>
                    </div>
                </div>
        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-8 shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden transition-all duration-300">
                ...
                <div class="space-y-8 relative z-10">
                    <div>
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Nama Ruas Jalan</p>
                        <p class="text-sm font-black text-blue-950 dark:text-white uppercase leading-relaxed"><?= $jalan['nama_jalan'] ?: 'RUAS TANPA NAMA' ?></p>
                    </div>

                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">ID Inventaris</p>
                            <p class="text-xs font-black text-slate-700 dark:text-slate-300"><?= $jalan['id_csv'] ?? '-' ?></p>
                        </div>
                        <div>
                            <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Skor Kondisi</p>
                            <p class="text-xs font-black text-indigo-600"><?= number_format($jalan['jalan'], 2) ?></p>
                        </div>
                    </div>

                    <div class="pt-6 border-t border-slate-50 dark:border-slate-800">
                        <div class="flex items-center justify-between mb-3">
                            <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Format WKT</p>
            ...
            <!-- Kartu Sistem Log -->
            <div class="bg-slate-900 rounded-[2rem] p-8 text-white shadow-xl relative overflow-hidden group transition-all hover:bg-slate-950">
