...
<div class="space-y-10">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-black text-blue-950 dark:text-white tracking-tight">Dashboard Kontrol</h1>
            <p class="text-slate-400 dark:text-slate-500 text-sm font-medium italic">Pusat informasi pemantauan data perumahan Kabupaten Sinjai.</p>
        </div>
        <div class="flex items-center gap-4">
            <!-- Signal Indicator -->
            <div id="ping-status" class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm px-4 py-2 rounded-2xl flex items-center gap-3 transition-colors duration-300">
                <div class="flex gap-0.5 items-end h-3">
                    <div class="w-1 bg-slate-200 dark:bg-slate-700 rounded-full h-1 bar-1 transition-all duration-500"></div>
                    <div class="w-1 bg-slate-200 dark:bg-slate-700 rounded-full h-2 bar-2 transition-all duration-500"></div>
                    <div class="w-1 bg-slate-200 dark:bg-slate-700 rounded-full h-3 bar-3 transition-all duration-500"></div>
                </div>
                <span id="ping-text" class="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Checking...</span>
            </div>
            <!-- Waktu & Tanggal Real-time -->
            <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm px-5 py-2 rounded-2xl flex items-center gap-4 transition-colors duration-300">
                <div class="flex items-center gap-2 border-r border-slate-100 dark:border-slate-800 pr-4">
                    <i data-lucide="calendar" class="w-3.5 h-3.5 text-slate-400"></i>
                    <span class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest"><?= date('d M Y') ?></span>
                </div>
                <div class="flex items-center gap-2">
                    <i data-lucide="clock" class="w-3.5 h-3.5 text-blue-600 dark:text-blue-400"></i>
                    <span id="digital-clock" class="text-[10px] font-black text-blue-900 dark:text-blue-400 tracking-[0.2em]">00:00:00</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-10">
        <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden transition-colors duration-300">
            <div class="p-6 border-b dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center">
                <div class="flex items-center gap-3">
                    <div class="w-2 h-2 bg-blue-600 dark:bg-blue-400 rounded-full"></div>
                    <h3 class="font-black text-blue-950 dark:text-white uppercase tracking-widest text-xs">Log Aktivitas Terbaru</h3>
                </div>
                <i data-lucide="activity" class="w-4 h-4 text-slate-300 dark:text-slate-600"></i>
            </div>
            <div class="divide-y divide-slate-50 dark:divide-slate-800">
                <?php if(empty($logs)): ?>
                    <div class="p-12 text-center text-slate-400 dark:text-slate-600 italic text-sm">
                        <i data-lucide="inbox" class="w-10 h-10 mx-auto mb-3 opacity-20 text-slate-900 dark:text-white"></i>
                        Belum ada aktivitas tercatat hari ini.
                    </div>
                <?php else: foreach($logs as $log): ?>
                    <div class="p-5 flex items-center space-x-5 hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-all duration-300 group cursor-default">
                        <div class="w-11 h-11 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 flex items-center justify-center shrink-0 shadow-sm group-hover:shadow-md transition-all">
                            <?php 
                        ...
                        <div class="flex-grow">
                            <p class="text-sm font-black text-slate-800 dark:text-slate-200 uppercase tracking-tight"><?= $log['action'] ?> Data <span class="text-blue-900 dark:text-blue-400"><?= $log['table_name'] ?></span></p>
                            <p class="text-xs text-slate-500 dark:text-slate-400 font-bold mt-1 flex items-center gap-2">
                                <span class="bg-blue-900 dark:bg-blue-700 text-white px-2 py-0.5 rounded text-[8px] uppercase tracking-tighter"><?= $log['user'] ?></span>
                                <span class="italic font-medium text-slate-400 dark:text-slate-500"><?= $log['description'] ?></span>
                            </p>
                        </div>
                        <div class="text-right shrink-0">
                            <!-- Waktu Sinkron: Hanya Jam & Menit -->
                            <p class="text-[11px] font-black text-blue-900 dark:text-blue-400 uppercase leading-none"><?= date('H:i', strtotime($log['created_at'])) ?></p>
                            <p class="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter mt-1"><?= date('d M', strtotime($log['created_at'])) ?></p>
                        </div>
                    </div>
...
<script>
    ...
    async function checkPing() {
        ...
        try {
            ...
            // Definisi class untuk mempermudah pembersihan
            const allBgStatus = ['bg-emerald-500', 'bg-amber-500', 'bg-rose-500', 'bg-slate-200', 'dark:bg-slate-700'];
            const allTextStatus = ['text-emerald-500', 'text-amber-500', 'text-rose-500', 'text-slate-400', 'dark:text-slate-600'];

            // Bersihkan semua batang & teks
            bars.forEach(b => b.classList.remove(...allBgStatus));
            pingText.classList.remove(...allTextStatus);
            
            if (latency < 150) {
                bars.forEach(b => b.classList.add('bg-emerald-500'));
                pingText.innerText = 'Signal: Excellent';
                pingText.classList.add('text-emerald-500');
            } else if (latency < 400) {
                bars[0].classList.add('bg-amber-500'); 
                bars[1].classList.add('bg-amber-500');
                bars[2].classList.add('bg-slate-200', 'dark:bg-slate-700');
                pingText.innerText = 'Signal: Good';
                pingText.classList.add('text-amber-500');
            } else {
                bars[0].classList.add('bg-rose-500');
                bars[1].classList.add('bg-slate-200', 'dark:bg-slate-700');
                bars[2].classList.add('bg-slate-200', 'dark:bg-slate-700');
                pingText.innerText = 'Signal: Poor';
                pingText.classList.add('text-rose-500');
            }
        } catch (e) {
            pingText.innerText = 'Signal: Offline';
            bars.forEach(b => {
                b.classList.remove('bg-emerald-500', 'bg-amber-500', 'bg-rose-500');
