...
<div class="max-w-7xl mx-auto space-y-6 pb-12 text-slate-900 dark:text-slate-200">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden transition-all duration-300">
        ...
        <div class="flex items-center gap-2 relative z-10 no-print">
            <button onclick="bansosModal.openEdit(<?= htmlspecialchars(json_encode($bansos), ENT_QUOTES, 'UTF-8') ?>)" class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-indigo-700 transition-all active:scale-95 flex items-center gap-2 shadow-lg shadow-indigo-600/20">
...
<script>
    function initDetailMap() {
        ...
        const map = L.map('map-detail', { zoomControl: false, layers: [googleSat] }).setView([lat, lng], 17);
