    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden">
        <div class="p-8 border-b dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-900/20">
                    <i data-lucide="edit-3" class="w-6 h-6"></i>
                </div>
                <div>
                    <h2 class="text-xl font-bold text-blue-950 dark:text-white uppercase tracking-tight">Edit Realisasi Bansos</h2>
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Perbarui data realisasi bantuan sesuai kondisi terkini</p>
                </div>
            </div>
            <a href="<?= base_url('bansos-rtlh/detail/' . $bansos['id']) ?>" class="px-5 py-2.5 bg-white dark:bg-slate-800 text-slate-500 rounded-xl text-[10px] font-bold uppercase tracking-widest border border-slate-200 dark:border-slate-700 hover:bg-slate-50 transition-all flex items-center gap-2">
                <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i> Batal
            </a>
        </div>

        <form action="<?= base_url('bansos-rtlh/update/' . $bansos['id']) ?>" method="POST" enctype="multipart/form-data" class="p-10 space-y-10">
            <?= csrf_field() ?>
            ...
            <div class="bg-blue-50/50 dark:bg-blue-950/20 p-6 rounded-3xl border border-blue-100/50 dark:border-blue-900/30">
                ...
                <select name="id_survei" id="id_survei" onchange="fillFromRtlh(this)" class="w-full p-4 bg-white dark:bg-slate-900 border border-blue-200 dark:border-blue-800 rounded-2xl focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-bold text-sm shadow-sm">
                    ...
                    <?php foreach($rtlh as $r): ?>
                        <option value="<?= $r['id_survei'] ?>" data-nik="<?= $r['nik'] ?>" data-nama="<?= $r['nama_kepala_keluarga'] ?>" data-desa="<?= $r['desa'] ?>" <?= $bansos['id_survei'] == $r['id_survei'] ? 'selected' : '' ?>>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="nik" id="nik" value="<?= $bansos['nik'] ?>" placeholder="Masukkan 16 digit NIK" class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 dark:text-white outline-none transition-all font-bold" required <?= $bansos['id_survei'] ? 'readonly' : '' ?>>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Nama Lengkap</label>
                        <input type="text" name="nama_penerima" id="nama_penerima" value="<?= $bansos['nama_penerima'] ?>" placeholder="Nama sesuai KTP" class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 dark:text-white outline-none transition-all font-bold" required <?= $bansos['id_survei'] ? 'readonly' : '' ?>>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Wilayah Desa</label>
                        <input type="text" name="desa" id="desa" value="<?= $bansos['desa'] ?>" placeholder="Nama Desa / Kelurahan" class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 dark:text-white outline-none transition-all font-bold" required <?= $bansos['id_survei'] ? 'readonly' : '' ?>>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Tahun Anggaran</label>
                        <input type="number" name="tahun_anggaran" value="<?= $bansos['tahun_anggaran'] ?>" min="2000" max="2099" class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 dark:text-white outline-none transition-all font-bold" required>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="sumber_dana" value="<?= $bansos['sumber_dana'] ?>" placeholder="Contoh: BSPS, APBD Sinjai, DAK Bidang Perumahan" class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 dark:text-white outline-none transition-all font-bold" required>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="lokasi_realisasi" id="lokasi_realisasi" value="<?= $bansos['wkt_realisasi'] ?>" placeholder="POINT(lng lat)" class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl font-mono text-xs font-bold text-emerald-600 outline-none" readonly>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-3">
                        ...
                        <div class="relative group aspect-video bg-slate-50 dark:bg-slate-950 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center overflow-hidden transition-all hover:border-emerald-500/50">
                            ...
                            <div id="placeholder-foto_before" class="flex flex-col items-center justify-center <?= !empty($bansos['foto_before']) ? 'hidden' : '' ?>">
                                <i data-lucide="camera" class="w-6 h-6 text-slate-300 mb-2"></i>
                                <span class="text-[9px] font-bold text-slate-400 uppercase">Ganti Foto Sebelum</span>
                            </div>
                            <img id="preview-foto_before" src="<?= !empty($bansos['foto_before']) ? base_url('uploads/rtlh/'.$bansos['foto_before']) : '' ?>" class="absolute inset-0 w-full h-full object-cover <?= !empty($bansos['foto_before']) ? '' : 'hidden' ?>">
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="relative group aspect-video bg-slate-50 dark:bg-slate-950 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center overflow-hidden transition-all hover:border-emerald-500/50">
                            ...
                            <div id="placeholder-foto_after" class="flex flex-col items-center justify-center <?= !empty($bansos['foto_after']) ? 'hidden' : '' ?>">
                                <i data-lucide="camera" class="w-6 h-6 text-slate-300 mb-2"></i>
                                <span class="text-[9px] font-bold text-slate-400 uppercase">Ganti Foto Sesudah</span>
                            </div>
                            <img id="preview-foto_after" src="<?= !empty($bansos['foto_after']) ? base_url('uploads/rtlh/'.$bansos['foto_after']) : '' ?>" class="absolute inset-0 w-full h-full object-cover <?= !empty($bansos['foto_after']) ? '' : 'hidden' ?>">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="keterangan" rows="3" placeholder="Informasi tambahan mengenai proses pembangunan atau kendala..." class="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 dark:text-white outline-none transition-all font-bold"><?= $bansos['keterangan'] ?></textarea>
            </div>

            <div class="pt-6">
                <button type="submit" class="w-full py-5 bg-blue-600 hover:bg-blue-700 text-white rounded-[2rem] text-sm font-bold uppercase tracking-[0.2em] shadow-xl shadow-blue-900/20 transition-all active:scale-[0.98] flex items-center justify-center gap-3">
...
<script>
    ...
    function initMap() {
        ...
        const wkt = "<?= $bansos['wkt_realisasi'] ?>";
        if (wkt) {
