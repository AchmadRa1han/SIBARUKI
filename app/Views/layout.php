    ...
    <script>
        ...
        function customConfirm(title, message, type = 'info') {
            return new Promise((resolve) => {
                ...
                modal.innerHTML = `<div class="fixed inset-0 bg-slate-900/60 backdrop-blur-md"></div><div class="relative bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-2xl overflow-hidden scale-95 transition-transform duration-300"><div class="p-8 text-center"><div class="w-16 h-16 bg-${colors.light} dark:bg-${colors.dark} text-${colors.text} rounded-2xl flex items-center justify-center mx-auto mb-6"><i data-lucide="${colors.icon}" class="w-8 h-8"></i></div><h3 class="text-xl font-black text-blue-950 dark:text-white uppercase tracking-tight mb-3">${title}</h3><p class="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed">${message}</p></div><div class="p-6 bg-slate-50/50 dark:bg-slate-950/50 border-t dark:border-slate-800 flex gap-3"><button id="confirm-cancel" class="flex-1 py-3 text-[10px] font-black text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest">Batal</button><button id="confirm-ok" class="flex-[2] py-3 bg-${colors.bg} text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all">${options.confirmText || 'Lanjutkan'}</button></div></div>`;
