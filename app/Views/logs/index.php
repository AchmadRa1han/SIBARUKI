...
<div class="space-y-8 pb-12 animate-in fade-in duration-700">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm group">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">24H Deleted</span>
            </div>
            <h3 class="text-3xl font-bold text-slate-800 dark:text-white"><?= number_format($snapshot['deleted']) ?></h3>
        ...
        <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm group">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">24H Exported</span>
            </div>
            <h3 class="text-3xl font-bold text-slate-800 dark:text-white"><?= number_format($snapshot['exported']) ?></h3>
        ...
        <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm group">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">24H Created</span>
            </div>
            <h3 class="text-3xl font-bold text-slate-800 dark:text-white"><?= number_format($snapshot['created']) ?></h3>
            <p class="text-[9px] font-bold text-emerald-500 uppercase mt-1 tracking-tighter italic">System Ingestion</p>
        </div>
        <div class="bg-rose-600 p-6 rounded-2xl shadow-xl shadow-rose-600/20 group text-white">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[8px] font-bold text-rose-200 uppercase tracking-widest">24H Critical</span>
            </div>
            <h3 class="text-3xl font-bold"><?= number_format($snapshot['critical']) ?></h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 bg-blue-950 rounded-3xl p-10 text-white relative overflow-hidden shadow-2xl group">
            <div class="absolute top-0 right-0 w-64 h-64 bg-blue-400/10 rounded-full blur-[80px] -mr-32 -mt-32"></div>
            <div class="relative z-10 flex flex-col h-full justify-between">
                <div class="flex items-center justify-between mb-10">
                    <div>
                        <h3 class="text-[10px] font-bold text-blue-300 uppercase tracking-[0.3em] mb-1">Aktivitas Sistem (Pulse)</h3>
                    ...
                    <div class="flex p-1 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10">
                        <?php foreach(['hourly', 'daily', 'monthly'] as $r): ?>
                            <button onclick="updateTrend('<?= $r ?>')" id="tab-<?= $r ?>" class="px-4 py-2 rounded-xl text-[9px] font-bold uppercase tracking-widest transition-all <?= $r=='hourly'?'bg-white text-blue-950 shadow-lg':'text-blue-300 hover:text-white' ?>"><?= ucfirst($r) ?></button>
                        <?php endforeach; ?>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-between">
            <div>
                <h3 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-8 flex items-center justify-between">
            ...
            <div class="mt-8 pt-6 border-t border-slate-50 dark:border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[10px] font-bold text-slate-800 dark:text-slate-200 uppercase">Mean Latency</p>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-2xl overflow-hidden">
        <div class="p-8 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-slate-50/50 to-white dark:from-slate-900/50 dark:to-slate-900">
            <form action="<?= base_url('logs') ?>" method="get" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                ...
                <div class="space-y-2">
                    <label class="text-[9px] font-bold text-rose-600 dark:text-rose-400 uppercase tracking-widest ml-1">Action</label>
                ...
                <div class="space-y-2">
                    <label class="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest ml-1">Severity</label>
                ...
                <div class="space-y-2">
                    <label class="text-[9px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest ml-1">Timestamp</label>
                ...
                <div class="flex items-end pb-0.5">
                    <a href="<?= base_url('logs') ?>" class="w-full flex items-center justify-center gap-2 py-3 bg-slate-100 dark:bg-slate-800 text-[10px] font-bold uppercase text-slate-500 hover:bg-rose-600 hover:text-white rounded-2xl transition-all shadow-sm group">
            ...
            <table class="w-full text-left border-collapse min-w-[1100px]">
                <thead>
                    <tr class="bg-white dark:bg-slate-900 border-b dark:border-slate-800">
                        <th class="p-8 text-[10px] font-bold text-blue-900 dark:text-blue-400 uppercase tracking-[0.2em]">Log Context</th>
                        <th class="p-8 text-[10px] font-bold text-blue-900 dark:text-blue-400 uppercase tracking-[0.2em]">Actor Forensic</th>
                        <th class="p-8 text-[10px] font-bold text-blue-900 dark:text-blue-400 uppercase tracking-[0.2em] text-center">Threat Level</th>
                        <th class="p-8 text-[10px] font-bold text-blue-900 dark:text-blue-400 uppercase tracking-[0.2em]">Target System</th>
                        <th class="p-8 text-[10px] font-bold text-blue-900 dark:text-blue-400 uppercase tracking-[0.2em] text-center">Audit Trails</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all group">
                        <td class="p-8">
                            <div class="flex flex-col">
                                <span class="text-[9px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest mb-1">Entry #<?= $log['id'] ?></span>
                                <p class="text-sm font-bold text-slate-700 dark:text-slate-200"><?= date('H:i:s', strtotime($log['created_at'])) ?></p>
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1"><?= date('d F Y', strtotime($log['created_at'])) ?></p>
                            </div>
                        </td>
                        <td class="p-8">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 flex items-center justify-center text-sm font-bold uppercase shadow-inner shrink-0"><?= substr($log['user'], 0, 1) ?></div>
                                <div class="min-w-0">
                                    <p class="text-sm font-bold text-slate-800 dark:text-slate-200 truncate"><?= $log['user'] ?></p>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[9px] font-bold text-blue-600 dark:text-blue-400 uppercase px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 rounded-md"><?= $log['ip_address'] ?></span>
                                        <?php if(!$isLegacy): ?>
                                    ...
                                    <div class="flex items-center gap-3 mt-3 opacity-60 group-hover:opacity-100 transition-opacity">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="text-[8px] font-bold text-slate-500 uppercase"><?= $meta['browser'] ?> <?= $meta['version'] ?></span>
                                        </div>
                                        <div class="flex items-center gap-1.5">
                                            <i data-lucide="clock" class="w-3 h-3 text-slate-400"></i>
                                            <span class="text-[8px] font-bold text-emerald-600 uppercase"><?= $meta['latency_ms'] ?>ms</span>
                                        </div>
                                    </div>
                                    <?php endif; ?>
                                </div>
                        ...
                        <td class="p-8 text-center">
                            ...
                            <div class="flex flex-col items-center gap-2">
                                <span class="px-3 py-1 rounded-full text-[8px] font-bold uppercase tracking-widest border <?= $sevClass ?>"><?= $log['severity'] ?></span>
                                <span class="text-[10px] font-bold text-slate-500 uppercase"><?= $log['action'] ?></span>
                            </div>
                        </td>
                        <td class="p-8">
                            <div class="space-y-1">
                                <span class="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest"><?= $log['table_name'] ?></span>
                                <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium italic line-clamp-2"><?= $log['description'] ?></p>
                            </div>
                        </td>
                        <td class="p-8 text-center">
                            <?php if(!empty($log['details'])): ?>
                                <button onclick="toggleLogDetail('detail-<?= $log['id'] ?>')" class="px-5 py-2.5 bg-blue-950 dark:bg-white text-white dark:text-blue-950 rounded-xl text-[9px] font-bold uppercase tracking-[0.2em] hover:scale-105 transition-all shadow-xl active:scale-95 whitespace-nowrap">Investigate</button>
                            <?php else: ?>
                    ...
                    <tr id="detail-<?= $log['id'] ?>" class="hidden bg-slate-50/50 dark:bg-slate-950/50">
                        <td colspan="5" class="p-10">
                            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-blue-100 dark:border-blue-900 overflow-hidden shadow-2xl">
                                <div class="p-6 bg-blue-950 text-white flex items-center justify-between">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p class="text-[9px] font-bold text-blue-400 uppercase tracking-widest mb-1">Audit Evidence Analysis</p>
                                            <h4 class="text-sm font-bold uppercase"><?= $log['action'] ?> Data - <?= $log['table_name'] ?></h4>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-[9px] font-bold text-blue-400 uppercase">Process Latency</p>
                                        <span class="text-xs font-bold text-emerald-400"><?= $meta['latency_ms'] ?? '?' ?> ms</span>
                                    </div>
                                </div>
                                    ...
                                    <div class="grid grid-cols-1 gap-4">
                                        ...
                                        ?>
                                            <div class="p-5 bg-slate-50/50 dark:bg-slate-950/50 rounded-2xl border border-transparent hover:border-slate-200 transition-all">
                                                ...
                                                ?>
                                                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                                                        <div class="w-40 shrink-0"><span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest"><?= $fieldName ?></span></div>
                                                        <div class="flex flex-grow items-center gap-4">
                                                            <div class="flex-1 bg-white dark:bg-slate-900 p-3 rounded-xl border border-rose-100 text-center relative overflow-hidden">
                                                                <span class="text-[8px] absolute top-1 left-2 font-bold text-rose-400 uppercase opacity-50">BEFORE</span>
                                                            ...
                                                            <div class="flex-1 bg-white dark:bg-slate-900 p-3 rounded-xl border border-emerald-100 shadow-md text-center relative overflow-hidden">
                                                                <span class="text-[8px] absolute top-1 left-2 font-bold text-emerald-400 uppercase">AFTER</span>
                                                                <span class="text-xs font-bold text-emerald-600 dark:text-emerald-400"><?= $newVal ?></span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                <?php else: ?>
                                                    <div class="flex items-center gap-4">
                                                        ...
                                                        <p class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tight"><?= $d ?></p>
                                                    </div>
                                                <?php endif; ?>
                                            </div>
                                        <?php endforeach; ?>
                                ...
                                <div class="p-6 bg-slate-50 dark:bg-slate-950 border-t dark:border-slate-800 flex justify-between items-center text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">
            ...
            <div class="flex items-center gap-6">
                <div class="flex items-center gap-3 bg-white dark:bg-slate-900 p-1 rounded-2xl shadow-inner border border-slate-100 dark:border-slate-800">
                    <?php foreach([10, 25, 50] as $count): ?>
                        <button onclick="changePerPage(<?= $count ?>)" class="px-4 py-2 rounded-xl text-[10px] font-bold uppercase transition-all <?= $perPage == $count ? 'bg-blue-950 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600' ?>"><?= $count ?></button>
                    <?php endforeach; ?>
                </div>
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Record: <span class="text-blue-600"><?= number_format($total) ?></span></span>
            </div>
...
<script>
    ...
    function updateTrend(range) {
        ['hourly', 'daily', 'monthly'].forEach(r => {
            document.getElementById(`tab-${r}`).className = `px-4 py-2 rounded-xl text-[9px] font-bold uppercase tracking-widest transition-all ${r===range?'bg-white text-blue-950 shadow-lg':'text-blue-300 hover:text-white'}`;
        });
