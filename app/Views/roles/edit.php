...
<div class="p-8 max-w-5xl mx-auto">
    <div class="mb-10">
        ...
        <h1 class="text-3xl font-black text-blue-950 uppercase tracking-tight">Edit Role: <?= strtoupper($role['role_name']) ?></h1>
            ...
            <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="text" name="role_name" value="<?= $role['role_name'] ?>" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 focus:ring-4 focus:ring-blue-900/10 focus:border-blue-900 outline-none transition-all font-bold text-slate-700 placeholder:font-medium" placeholder="Contoh: Verifikator Lapangan" required>
                    </div>
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label class="cursor-pointer group">
                                <input type="radio" name="scope" value="global" class="hidden peer" <?= $role['scope'] == 'global' ? 'checked' : '' ?>>
                            ...
                            <label class="cursor-pointer group">
                                <input type="radio" name="scope" value="local" class="hidden peer" <?= $role['scope'] == 'local' ? 'checked' : '' ?>>
                                <div class="p-4 rounded-2xl border-2 border-slate-50 group-hover:border-blue-900/20 peer-checked:border-blue-900 peer-checked:bg-blue-50 transition-all text-center">
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
                            ...
                            <tr class="hover:bg-blue-50/30 transition-colors">
                                ...
                                <?php foreach ($actions as $action) : ?>
                                    <td class="p-6 text-center">
                                        ...
                                        ?>
                                        <?php if ($perm) : ?>
                                            <input type="checkbox" name="permissions[]" value="<?= $perm['id'] ?>" class="w-6 h-6 rounded-lg border-2 border-slate-200 text-blue-900 focus:ring-blue-900/20 cursor-pointer transition-all" <?= in_array($perm['id'], $activePerms) ? 'checked' : '' ?>>
                                        <?php else : ?>
                            ...
                            <tr class="hover:bg-amber-50/30 transition-colors">
                                ...
                                <?php foreach ($actions as $action) : ?>
                                    <td class="p-6 text-center">
                                        ...
                                        ?>
                                        <?php if ($perm) : ?>
                                            <input type="checkbox" name="permissions[]" value="<?= $perm['id'] ?>" class="w-6 h-6 rounded-lg border-2 border-slate-200 text-amber-600 focus:ring-amber-500/20 cursor-pointer transition-all" <?= in_array($perm['id'], $activePerms) ? 'checked' : '' ?>>
                                        <?php else : ?>
                                            <span class="text-slate-200">-</span>
                                        <?php endif; ?>
                                    </td>
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-colors">
                                ...
                                <?php foreach ($actions as $action) : ?>
                                    <td class="p-6 text-center">
                                        ...
                                        ?>
                                        <?php if ($perm) : ?>
                                            <input type="checkbox" name="permissions[]" value="<?= $perm['id'] ?>" class="w-6 h-6 rounded-lg border-2 border-slate-200 text-indigo-600 focus:ring-indigo-500/20 cursor-pointer transition-all" <?= in_array($perm['id'], $activePerms) ? 'checked' : '' ?>>
                                        <?php else : ?>
                                            <span class="text-slate-200">-</span>
                                        <?php endif; ?>
                                    </td>
                ...
                <div class="p-8 bg-slate-50/50">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <?php 
                            $systemPerms = ['manage_roles', 'export_data'];
                        ...
                        ?>
                            <label class="flex items-center gap-3 p-4 bg-white rounded-2xl border border-slate-100 hover:border-blue-900/20 cursor-pointer transition-all group">
                                <input type="checkbox" name="permissions[]" value="<?= $p['id'] ?>" class="w-5 h-5 rounded-lg border-2 border-slate-200 text-blue-900 focus:ring-blue-900/20 cursor-pointer" <?= in_array($p['id'], $activePerms) ? 'checked' : '' ?>>
