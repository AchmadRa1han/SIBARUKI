...
<div class="space-y-6 pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap md:flex-nowrap items-center justify-end gap-2 flex-shrink-0">
            ...
            <a href="<?= base_url('aset-tanah/export-excel') ?>" class="bg-emerald-50 text-emerald-600 px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all flex items-center gap-2 whitespace-nowrap">
                <i data-lucide="download" class="w-3 h-3"></i> Export Excel
            </a>
            <?php if (has_permission('create_rtlh')): ?>
            <a href="<?= base_url('aset-tanah/create') ?>" class="bg-blue-950 hover:bg-black text-white px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg transition-all flex items-center gap-2 group whitespace-nowrap">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div id="bulk-action-bar" class="absolute top-0 left-0 right-0 z-50 bg-blue-950 text-white p-4 transform -translate-y-full transition-transform duration-300 flex items-center justify-between px-10">
            <div class="flex items-center gap-4">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest opacity-70">Aksi massal untuk data aset tanah</p>
            </div>
        ...
        <div class="p-6 border-b border-slate-50 dark:border-slate-800 flex flex-col lg:flex-row justify-between items-center gap-6">
            ...
            <form action="<?= base_url('aset-tanah') ?>" method="get" class="flex flex-col md:flex-row items-center gap-2 w-full lg:w-auto" id="filter-form">
                ...
                <div class="relative w-full md:w-56">
                    <input type="text" name="search" value="<?= $search ?>" placeholder="Cari aset..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[9px] font-black px-3 py-2.5 pl-9 focus:ring-2 focus:ring-blue-500">
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[9px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-4 py-4 w-28 cursor-pointer hover:text-blue-600" onclick="applySort('no_sertifikat')">Sertifikat</th>
                        <th class="px-4 py-4 w-64 cursor-pointer hover:text-blue-600" onclick="applySort('nama_pemilik')">Nama Pemilik</th>
                        <th class="px-4 py-4 w-24 cursor-pointer hover:text-blue-600" onclick="applySort('luas_m2')">Luas</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[10px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-200">
                        ...
                        <td class="px-4 py-3"><span class="font-black text-blue-600 dark:text-blue-400 uppercase"><?= $item['no_sertifikat'] ?></span></td>
                        <td class="px-4 py-3">
                            <div class="flex flex-col gap-0.5">
                                ...
                                <span class="text-[8px] text-slate-400 font-bold uppercase truncate"><?= $item['lokasi'] ?></span>
                            </div>
                        </td>
                        <td class="px-4 py-3"><div class="flex items-center gap-1"><span class="font-black text-slate-700 dark:text-slate-300"><?= number_format($item['luas_m2'], 0, ',', '.') ?></span><span class="text-[7px] font-black text-slate-400">M²</span></div></td>
                        <td class="px-4 py-3"><span class="font-bold text-slate-500 dark:text-slate-400 uppercase truncate block"><?= $item['kecamatan'] ?></span></td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1.5">
                                ...
                                <?php endif; ?>
                                <a href="<?= base_url('aset-tanah/detail/'.$item['id']) ?>" class="p-2 bg-blue-950 text-white rounded-lg hover:bg-black transition-all active:scale-95"><i data-lucide="eye" class="w-3.5 h-3.5"></i></a>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (mc) {
            ...
            document.querySelectorAll('nav a').forEach(link => link.addEventListener('click', () => localStorage.setItem('asetTanahScrollPos', mc.scrollTop)));
        }
    });
    ...
    async function handleBulkDelete() {
        ...
        const ok = await window.customConfirm('Hapus Massal?', `Apakah Anda yakin ingin menghapus ${ids.length} data aset yang dipilih? Tindakan ini tidak dapat dibatalkan.`, 'danger');
        ...
        if (ok) {
            ...
            try {
                const response = await fetch('<?= base_url('aset-tanah/bulk-delete') ?>', {
                    method: 'POST',
                    body: formData,
