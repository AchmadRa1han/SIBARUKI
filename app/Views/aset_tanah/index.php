...
<div class="space-y-8 pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h1 class="text-3xl md:text-4xl font-black text-blue-950 dark:text-white uppercase tracking-tighter">Aset Tanah Pemda</h1>
            <p class="text-slate-500 dark:text-slate-400 font-medium text-sm mt-1">Manajemen & monitoring aset tanah Pemerintah Daerah Kabupaten Sinjai.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3 relative z-10">
            <div class="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-blue-100 dark:border-blue-800/50 shadow-sm">
                <?= number_format($total_aset ?? 0) ?> Bidang Terdata
            </div>
            <a href="<?= base_url('aset-tanah/export-excel') ?>" class="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50 hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center gap-3 shadow-sm">
                <i data-lucide="download" class="w-4 h-4"></i> Export Excel
            </a>
            <?php if (has_permission('create_rtlh')): ?>
            <a href="<?= base_url('aset-tanah/create') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-blue-600/20 transition-all active:scale-95 flex items-center gap-3 group">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800">
            ...
            <div class="absolute top-8 left-8 z-[1000] hidden md:block">
                <div class="bg-blue-950/80 backdrop-blur-md text-white px-5 py-3 rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl border border-white/10 flex items-center gap-3">
                    ...
                    Database Pertanahan Terintegrasi
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-6">
            <div class="flex items-center gap-4 w-full lg:w-auto">
                <div class="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl w-full md:w-auto">
                    <button class="px-6 py-2.5 bg-white dark:bg-slate-700 text-blue-600 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">Daftar Sertifikat</button>
                </div>
            </div>

            <form action="<?= base_url('aset-tanah') ?>" method="get" class="flex flex-col md:flex-row items-center gap-3 w-full lg:w-auto" id="filter-form">
                ...
                <div class="relative w-full md:w-48">
                    <select name="kecamatan" onchange="submitWithScroll(this)" class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-3 focus:ring-2 focus:ring-blue-500 cursor-pointer appearance-none">
                ...
                <div class="relative w-full md:w-64">
                    <input type="text" name="search" value="<?= $search ?>" placeholder="Cari aset..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-3 pl-11 focus:ring-2 focus:ring-blue-500 transition-all">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div id="bulk-action-bar" class="absolute top-0 left-0 right-0 z-50 bg-blue-950 text-white p-5 transform -translate-y-full transition-transform duration-500 flex items-center justify-between px-10">
            <div class="flex items-center gap-5">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest opacity-70 hidden md:block">Aksi massal untuk data aset tanah</p>
            </div>
        ...
        <div class="p-8 border-b border-slate-50 dark:border-slate-800">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-lg font-black text-blue-950 dark:text-white uppercase tracking-tight">Inventaris Aset Tanah</h3>
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Data Legalitas & Luas Bidang</p>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-4 py-5 w-36 cursor-pointer hover:text-blue-600 transition-colors" onclick="applySort('no_sertifikat')">
                            No. Sertifikat
                        </th>
                        <th class="px-4 py-5 w-64 cursor-pointer hover:text-blue-600 transition-colors" onclick="applySort('nama_pemilik')">
                            Nama Pemilik / Instansi
                        </th>
                        <th class="px-4 py-5 w-32 text-center cursor-pointer hover:text-blue-600 transition-colors" onclick="applySort('luas_m2')">
                            Luas (M²)
                        </th>
                        <th class="px-4 py-5 w-48">Kecamatan</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[11px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-300">
                        <td class="px-8 py-4 text-center">
                            <input type="checkbox" name="ids[]" value="<?= $item['id'] ?>" class="row-checkbox w-5 h-5 rounded-lg border-2 border-slate-200 text-blue-600 focus:ring-blue-600/20 cursor-pointer transition-all">
                        </td>
                        <td class="px-4 py-4">
                        ...
                        <td class="px-4 py-4">
                            <div class="flex flex-col gap-0.5">
                                <span class="font-black text-blue-950 dark:text-white uppercase truncate block text-sm mb-0.5" title="<?= $item['nama_pemilik'] ?>"><?= $item['nama_pemilik'] ?></span>
                                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate"><?= $item['lokasi'] ?></span>
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <button onclick="focusMap('<?= $item['koordinat'] ?>')" class="p-2.5 bg-white dark:bg-slate-800 text-blue-600 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 hover:bg-blue-600 hover:text-white transition-all active:scale-95" title="Lihat di Peta"><i data-lucide="map-pin" class="w-4 h-4"></i></button>
                                <?php endif; ?>
                                <a href="<?= base_url('aset-tanah/detail/'.$item['id']) ?>" class="p-2.5 bg-blue-950 dark:bg-blue-600 text-white rounded-xl shadow-md hover:scale-110 transition-all active:scale-95" title="Detail Data"><i data-lucide="eye" class="w-4 h-4"></i></a>
        ...
        try {
            ...
            asetData.forEach(item => {
                if (item.koordinat) {
                    ...
                    marker.bindPopup(`
                        <div class="bg-blue-950 text-white p-4 rounded-t-xl border-b border-white/10"><p class="text-[8px] font-black uppercase tracking-[0.2em] text-blue-400 mb-1">Aset Tanah</p><h5 class="text-xs font-black uppercase leading-tight">${item.nama_pemilik}</h5></div>
                        <div class="p-4 bg-white dark:bg-slate-900 space-y-3 rounded-b-xl"><p class="text-[10px] font-black text-blue-600 uppercase">${item.no_sertifikat}</p><a href="<?= base_url('aset-tanah/detail/') ?>/${item.id}" class="block w-full py-2 bg-blue-950 text-white text-center text-[9px] font-black uppercase tracking-widest rounded-xl transition-all">Detail</a></div>
                    `);
