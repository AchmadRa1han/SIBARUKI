    ...
    function switchLayer(type) {
        ...
        items.forEach(item => {
            try {
                ...
                const detailLink = `<a href="${detailUrls[type]}/${item.id}" class="block w-full py-2.5 bg-blue-950 hover:bg-blue-800 text-white text-center text-[10px] font-black uppercase tracking-[0.2em] rounded-xl shadow-xl transition-all">Detail Lengkap</a>`;
...
<!-- FLOATING COMPLAINT BUTTON (FAB) -->
<div class="fixed bottom-6 right-6 z-[9999] no-print">
    <a href="https://wa.me/6282292566762?text=Halo%20Admin%20SIBARUKI,%20saya%20ingin%20melaporkan%20pengaduan%20layanan%20RTLH%20(Rumah%20Tidak%20Layak%20Huni)." target="_blank" class="group relative flex items-center justify-center w-14 h-14 bg-gradient-to-tr from-emerald-500 to-green-600 text-white rounded-full shadow-[0_10px_25px_-5px_rgba(16,185,129,0.4),0_8px_10px_-6px_rgba(16,185,129,0.4)] hover:shadow-[0_20px_25px_-5px_rgba(16,185,129,0.5),0_10px_10px_-5px_rgba(16,185,129,0.5)] hover:scale-110 active:scale-95 transition-all duration-300">
        <span class="absolute right-16 bg-slate-900/90 backdrop-blur-sm text-white text-[9px] font-black uppercase tracking-widest px-3.5 py-2 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none whitespace-nowrap shadow-xl border border-white/10">Layanan Pengaduan RTLH</span>
        <!-- Pulse effect ring -->
