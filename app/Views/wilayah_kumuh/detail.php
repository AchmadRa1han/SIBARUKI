        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-800 relative group lg:h-[650px]">
                ...
                <div class="absolute bottom-6 right-6 z-[1000] flex flex-col gap-2">
                    <button onclick="map.fitBounds(polyLayer.getBounds(), {padding:[50,50]})" class="p-3 bg-white dark:bg-slate-900 rounded-xl shadow-2xl text-blue-600 hover:scale-110 active:scale-95 transition-all border border-slate-100 dark:border-slate-800">
...
<script>
    ...
    function initMap() {
        ...
        try {
            ...
            map = L.map('map', { zoomControl: false, layers: [googleSat] }).setView(coords[0], 17);
            ...
            if (coords.length > 1) {
                polyLayer = L.polygon(coords, { color: color, weight: 3, fillOpacity: 0.4, fillColor: color }).addTo(map);
                map.fitBounds(polyLayer.getBounds(), { padding: [50, 50], animate: true, duration: 1.5 });
            } else {
                polyLayer = L.circleMarker(coords[0], { radius: 12, fillColor: color, color: '#fff', weight: 3, fillOpacity: 1 }).addTo(map);
