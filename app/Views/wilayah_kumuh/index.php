...
<div class="space-y-8 pb-12">
    ...
    <nav class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 no-print">
        ...
        <span class="text-blue-600">Wilayah Kumuh</span>
    </nav>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h1 class="text-3xl md:text-4xl font-black text-blue-950 dark:text-white uppercase tracking-tighter">Wilayah Kumuh</h1>
            <p class="text-slate-500 dark:text-slate-400 font-medium text-sm mt-1">Monitoring dan pemetaan kawasan permukiman kumuh Kabupaten Sinjai.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3 relative z-10">
            <div id="debug-status" class="bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-rose-100 dark:border-rose-800/50 shadow-sm">
                Memuat Peta...
            </div>
            <a href="<?= base_url('wilayah-kumuh/export-excel') ?>" class="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50 hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center gap-3 shadow-sm">
                <i data-lucide="download" class="w-4 h-4"></i> Export Excel
            </a>
            <?php if (has_permission('create_kumuh')): ?>
            <a href="<?= base_url('wilayah-kumuh/create') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-blue-600/20 transition-all active:scale-95 flex items-center gap-3 group">
                <i data-lucide="plus" class="w-5 h-5 group-hover:rotate-90 transition-transform"></i> Tambah Kawasan
            </a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="relative">
        <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800">
            ...
            <!-- Floating Legend -->
            <div class="absolute bottom-8 left-8 z-[1000] bg-white/90 dark:bg-slate-950/90 backdrop-blur-md p-6 rounded-[2rem] shadow-2xl border border-white/20 dark:border-slate-800 w-64">
                <h4 class="text-[10px] font-black text-blue-950 dark:text-white uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                    <i data-lucide="layers" class="w-4 h-4 text-rose-600"></i> Kategori Kumuh
                </h4>
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-3 h-3 rounded-full bg-[#f43f5e] shadow-lg shadow-rose-200"></div>
                            <span class="text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase">Berat</span>
                        </div>
                        <span class="text-[9px] font-bold text-slate-400 font-mono">> 60</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-3 h-3 rounded-full bg-[#f97316] shadow-lg shadow-orange-200"></div>
                            <span class="text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase">Sedang</span>
                        </div>
                        <span class="text-[9px] font-bold text-slate-400 font-mono">40-60</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-3 h-3 rounded-full bg-[#f59e0b] shadow-lg shadow-amber-200"></div>
                            <span class="text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase">Ringan</span>
                        </div>
                        <span class="text-[9px] font-bold text-slate-400 font-mono">< 40</span>
                    </div>
                </div>
            </div>
            ...
            <div class="absolute top-8 left-8 z-[1000] hidden md:block">
                <div class="bg-blue-950/80 backdrop-blur-md text-white px-5 py-3 rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl border border-white/10 flex items-center gap-3">
                    ...
                    Pemetaan Geospasial Wilayah Kumuh
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-6">
            <div class="flex items-center gap-4 w-full lg:w-auto">
                <div class="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl w-full md:w-auto">
                    <button class="px-6 py-2.5 bg-white dark:bg-slate-700 text-blue-600 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">Daftar Kawasan</button>
                </div>
            </div>
            ...
            <form action="" method="get" class="flex flex-col md:flex-row items-center gap-3 w-full lg:w-auto" id="filter-form">
                ...
                <div class="relative w-full md:w-72">
                    <input type="text" name="keyword" value="<?= service('request')->getGet('keyword') ?? '' ?>" placeholder="Cari Kelurahan / Kawasan..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-3 pl-11 focus:ring-2 focus:ring-blue-500 transition-all">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div id="bulk-action-bar" class="absolute top-0 left-0 right-0 z-50 bg-blue-950 text-white p-5 transform -translate-y-full transition-transform duration-500 flex items-center justify-between px-10">
            <div class="flex items-center gap-5">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest opacity-70 hidden md:block">Aksi massal untuk data kawasan kumuh</p>
            </div>
            <div class="flex items-center gap-3">
        ...
        <div class="p-8 border-b border-slate-50 dark:border-slate-800">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-rose-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-rose-600/20">
                    <i data-lucide="database" class="w-6 h-6"></i>
                </div>
                <div>
                    <h3 class="text-lg font-black text-blue-950 dark:text-white uppercase tracking-tight">Database Wilayah Kumuh</h3>
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Parameter Teknis & Luas Delineasi</p>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-4 py-5 w-64">Lokasi Administratif</th>
                        <th class="px-4 py-5 w-48">Nama Kawasan</th>
                        <th class="px-4 py-5 w-32 text-center">Luas (Ha)</th>
                        <th class="px-4 py-5 w-32 text-center">Skor Kumuh</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[11px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-300">
                        <td class="px-8 py-4 text-center">
                            <input type="checkbox" name="ids[]" value="<?= $item['FID'] ?>" class="row-checkbox w-5 h-5 rounded-lg border-2 border-slate-200 text-blue-600 focus:ring-blue-600/20 cursor-pointer transition-all">
                        </td>
                        <td class="px-4 py-4">
                            <span class="font-black text-blue-950 dark:text-white uppercase truncate block text-sm mb-0.5"><?= $item['Kelurahan'] ?></span>
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Kec. <?= $item['Kecamatan'] ?></span>
                        </td>
                        <td class="px-4 py-4">
                            <span class="font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight"><?= $item['Kawasan'] ?: '-' ?></span>
                        </td>
                        <td class="px-4 py-4 text-center">
                            <span class="px-4 py-1.5 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl font-black text-[10px] border border-slate-100 dark:border-slate-700 shadow-sm">
                                <?= number_format($item['Luas_kumuh'] ?? 0, 2, ',', '.') ?> Ha
                            </span>
                        </td>
                        <td class="px-4 py-4 text-center">
                            <?php 
                                $skor = (float)($item['skor_kumuh'] ?? 0);
                                $colorClass = $skor >= 60 ? 'bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-950/30 dark:text-rose-400 dark:border-rose-900' : ($skor >= 40 ? 'bg-orange-50 text-orange-600 border-orange-100 dark:bg-orange-950/30 dark:text-orange-400 dark:border-orange-900' : 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-900');
                            ?>
                            <span class="px-4 py-1.5 rounded-xl border text-[10px] font-black <?= $colorClass ?>"><?= $skor ?></span>
                        </td>
                        <td class="px-8 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <?php if(!empty($item['WKT'])): ?>
                                <button onclick="focusMap('<?= $item['FID'] ?>')" class="p-2.5 bg-white dark:bg-slate-800 text-rose-600 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 hover:bg-rose-600 hover:text-white transition-all active:scale-95" title="Fokus Peta"><i data-lucide="map-pin" class="w-4 h-4"></i></button>
                                <?php endif; ?>
                                <a href="<?= base_url('wilayah-kumuh/detail/' . $item['FID']) ?>" class="p-2.5 bg-blue-950 dark:bg-blue-600 text-white rounded-xl shadow-md hover:scale-110 transition-all active:scale-95" title="Detail Data"><i data-lucide="eye" class="w-4 h-4"></i></a>
                                <?php if (has_permission('delete_kumuh')): ?>
                                <button onclick="confirmDelete('<?= $item['FID'] ?>', '<?= addslashes($item['Kelurahan']) ?>')" class="p-2.5 bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 rounded-xl hover:bg-rose-600 hover:text-white transition-all active:scale-95" title="Hapus Data"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                                <?php endif; ?>
                            </div>
                        </td>
                    </tr>
...
<script>
                ...
                if (item.WKT && typeof wellknown !== 'undefined') {
                    try {
                        ...
                        if (geojson) {
                            ...
                            const popupContent = `
                                <div class="bg-blue-950 text-white p-4 rounded-t-xl"><p class="text-[8px] font-black uppercase tracking-widest text-rose-400 mb-1">Kawasan Kumuh</p><h5 class="text-xs font-black uppercase leading-tight">${item.Kelurahan}</h5></div>
                                <div class="p-4 bg-white dark:bg-slate-900 space-y-3 rounded-b-xl"><div class="flex justify-between items-center"><span class="text-[9px] font-bold text-slate-400 uppercase">Skor</span><span class="px-2 py-0.5 rounded-md bg-rose-50 dark:bg-rose-900/30 text-rose-600 text-[10px] font-black">${item.skor_kumuh}</span></div><a href="<?= base_url('wilayah-kumuh/detail/') ?>/${item.FID}" class="block w-full py-2 bg-blue-950 hover:bg-black text-white text-center text-[9px] font-black uppercase tracking-widest rounded-xl transition-all">Detail</a></div>
                            `;
                            const polyLayer = L.geoJSON(geojson, { style: { color: color, weight: 2, fillOpacity: 0.45, fillColor: color }, interactive: true }).bindPopup(popupContent);
                            polyLayer.addTo(map);
    ...
    async function handleBulkDelete() {
        ...
        const ok = await window.customConfirm('Hapus Massal?', `Hapus ${ids.length} data wilayah kumuh yang dipilih?`, 'danger');
        if (ok) {
            const formData = new FormData();
            ids.forEach(id => formData.append('ids[]', id));
            formData.append('<?= csrf_token() ?>', '<?= csrf_hash() ?>');
            try {
                const response = await fetch('<?= base_url('wilayah-kumuh/bulk-delete') ?>', { method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest' } });
