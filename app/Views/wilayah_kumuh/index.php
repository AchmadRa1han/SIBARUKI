...
<div class="space-y-6 pb-12 text-slate-900 dark:text-slate-200">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden transition-all duration-300">
        ...
        <div class="flex flex-wrap items-center gap-2 relative z-10">
            ...
            <a href="<?= base_url('wilayah-kumuh/export-excel') ?>" class="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50 hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center gap-2 shadow-sm">
                <i data-lucide="download" class="w-3.5 h-3.5"></i> Export
            </a>
            <?php if (has_permission('create_kumuh')): ?>
            <a href="<?= base_url('wilayah-kumuh/create') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-blue-600/20 transition-all active:scale-95 flex items-center gap-2 group">
    ...
    <div class="relative">
        <div class="bg-white dark:bg-slate-900 rounded-[2rem] overflow-hidden shadow-md border border-slate-100 dark:border-slate-800">
            ...
            <!-- Floating Legend -->
            <div class="absolute bottom-6 left-6 z-[1000] bg-white/90 dark:bg-slate-950/90 backdrop-blur-md p-5 rounded-2xl shadow-2xl border border-white/20 dark:border-slate-800 w-56">
                <h4 class="text-[9px] font-black text-blue-950 dark:text-white uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                    <i data-lucide="layers" class="w-3.5 h-3.5 text-rose-600"></i> Kategori Kumuh
                </h4>
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            <div class="w-2.5 h-2.5 rounded-full bg-[#f43f5e]"></div>
                            <span class="text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase">Berat</span>
                        </div>
                        <span class="text-[8px] font-bold text-slate-400 font-mono">> 60</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            <div class="w-2.5 h-2.5 rounded-full bg-[#f97316]"></div>
                            <span class="text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase">Sedang</span>
                        </div>
                        <span class="text-[8px] font-bold text-slate-400 font-mono">40-60</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            <div class="w-2.5 h-2.5 rounded-full bg-[#f59e0b]"></div>
                            <span class="text-[9px] font-black text-slate-700 dark:text-slate-300 uppercase">Ringan</span>
                        </div>
                        <span class="text-[8px] font-bold text-slate-400 font-mono">< 40</span>
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-4">
            <div class="flex items-center gap-3 w-full lg:w-auto">
                <div class="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-full md:w-auto">
                    <button class="px-4 py-2 bg-white dark:bg-slate-700 text-blue-600 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm">Daftar Kawasan</button>
                </div>
            </div>

            <form action="" method="get" class="flex flex-col md:flex-row items-center gap-2 w-full lg:w-auto" id="filter-form">
                ...
                <div class="relative w-full md:w-64">
                    <input type="text" name="keyword" value="<?= service('request')->getGet('keyword') ?? '' ?>" placeholder="Cari Kelurahan / Kawasan..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[9px] font-black uppercase px-3 py-2 pl-10 focus:ring-2 focus:ring-blue-500 transition-all">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div class="p-6 border-b border-slate-50 dark:border-slate-800">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-black text-blue-950 dark:text-white uppercase tracking-tight">Database Wilayah Kumuh</h3>
                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-[0.2em]">Parameter Teknis & Luas Delineasi</p>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[9px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-4 py-4 w-64">Lokasi Administratif</th>
                        <th class="px-4 py-4 w-48">Nama Kawasan</th>
                        <th class="px-4 py-4 w-32 text-center">Luas (Ha)</th>
                        <th class="px-4 py-4 w-32 text-center">Skor Kumuh</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[10px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-200">
                        <td class="px-6 py-3 text-center">
                            <input type="checkbox" name="ids[]" value="<?= $item['FID'] ?>" class="row-checkbox w-4.5 h-4.5 rounded-lg border-2 border-slate-200 text-blue-600 focus:ring-blue-600/20 cursor-pointer transition-all">
                        </td>
                        <td class="px-4 py-3">
                            <span class="font-black text-blue-950 dark:text-white uppercase truncate block text-xs mb-0.5"><?= $item['Kelurahan'] ?></span>
                            <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Kec. <?= $item['Kecamatan'] ?></span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight"><?= $item['Kawasan'] ?: '-' ?></span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-3 py-1 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg font-black text-[9px] border border-slate-100 dark:border-slate-700 shadow-sm">
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            ?>
                            <span class="px-3 py-1 rounded-lg border text-[9px] font-black <?= $colorClass ?>"><?= $skor ?></span>
                        </td>
                        <td class="px-6 py-3 text-center">
...
<script>
    ...
    function initMap() {
        ...
        try {
                ...
                if (item.WKT && typeof wellknown !== 'undefined') {
                    try {
                        ...
                        if (geojson) {
                            ...
                            const popupContent = `
                                <div class="bg-blue-950 text-white p-3 rounded-t-xl"><p class="text-[7px] font-black uppercase tracking-widest text-rose-400 mb-1">Kumuh</p><h5 class="text-[11px] font-black uppercase leading-tight">${item.Kelurahan}</h5></div>
                                <div class="p-3 bg-white dark:bg-slate-900 space-y-2 rounded-b-xl"><div class="flex justify-between items-center"><span class="text-[8px] font-bold text-slate-400 uppercase">Skor</span><span class="px-2 py-0.5 rounded-md bg-rose-50 dark:bg-rose-900/30 text-rose-600 text-[10px] font-black">${item.skor_kumuh}</span></div><a href="<?= base_url('wilayah-kumuh/detail/') ?>/${item.FID}" class="block w-full py-2 bg-blue-950 hover:bg-black text-white text-center text-[8px] font-black uppercase tracking-widest rounded-lg transition-all">Detail</a></div>
                            `;
                            const polyLayer = L.geoJSON(geojson, { style: { color: color, weight: 2, fillOpacity: 0.4, fillColor: color }, interactive: true }).bindPopup(popupContent);
                            polyLayer.addTo(map);
            ...
            debug.className = "bg-emerald-50 text-emerald-600 px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm border border-emerald-100";
    ...
    async function handleBulkDelete() {
        ...
        const ok = await window.customConfirm('Hapus Massal?', `Apakah Anda yakin ingin menghapus ${ids.length} data wilayah kumuh yang dipilih?`, 'danger');
        if (ok) {
...
<style>
    ...
    .marker-cluster-small div, .marker-cluster-medium div, .marker-cluster-large div { background-color: rgba(244, 63, 94, 0.85); color: white; font-weight: 900; font-size: 10px; }
</style>
<?= $this->endSection() ?>
