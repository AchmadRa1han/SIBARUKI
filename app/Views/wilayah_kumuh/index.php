    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div id="bulk-action-bar" class="absolute top-0 left-0 right-0 z-50 bg-blue-950 text-white p-4 transform -translate-y-full transition-transform duration-300 flex items-center justify-between px-10">
            <div class="flex items-center gap-4">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest opacity-70">Aksi massal untuk data wilayah kumuh</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-300">
                        <td class="px-6 py-3 text-center">
                            <input type="checkbox" name="ids[]" value="<?= $item['FID'] ?>" class="row-checkbox w-5 h-5 rounded-lg border-2 border-slate-200 text-blue-950 focus:ring-blue-900/20 cursor-pointer transition-all">
                        </td>
                        <td class="px-8 py-5">
...
<script>
    ...
    async function handleBulkDelete() {
        ...
        const ok = await window.customConfirm('Hapus Massal?', `Apakah Anda yakin ingin menghapus ${ids.length} data wilayah kumuh yang dipilih? Tindakan ini tidak dapat dibatalkan.`, 'danger');
