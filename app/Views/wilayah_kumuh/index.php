...
<div class="space-y-6 pb-12 text-slate-900 dark:text-slate-200">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden transition-all duration-300">
        ...
        <div class="relative z-10 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-blue-950 dark:text-white uppercase tracking-tighter">Wilayah Kumuh</h1>
        ...
        <div class="flex flex-wrap items-center gap-2 relative z-10">
            <div id="debug-status" class="bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 px-4 py-2 rounded-xl text-[9px] font-bold uppercase tracking-widest border border-rose-100 dark:border-rose-800/50 shadow-sm">
                Memuat Peta...
            </div>
            <a href="<?= base_url('wilayah-kumuh/export-excel') ?>" class="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-4 py-2 rounded-xl text-[9px] font-bold uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50 hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center gap-2 shadow-sm">
                <i data-lucide="download" class="w-3.5 h-3.5"></i> Export
            </a>
            <?php if (has_permission('create_kumuh')): ?>
            <a href="<?= base_url('wilayah-kumuh/create') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-[9px] font-bold uppercase tracking-widest shadow-lg shadow-blue-600/20 transition-all active:scale-95 flex items-center gap-2 group">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            ...
                            <span class="text-[9px] font-bold text-slate-700 dark:text-slate-300 uppercase">Berat</span>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            ...
                            <span class="text-[9px] font-bold text-slate-700 dark:text-slate-300 uppercase">Sedang</span>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            ...
                            <span class="text-[9px] font-bold text-slate-700 dark:text-slate-300 uppercase">Ringan</span>
                        </div>
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-4">
            <div class="flex items-center gap-3 w-full lg:w-auto">
                <div class="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-full md:w-auto">
                    <button class="px-4 py-2 bg-white dark:bg-slate-700 text-blue-600 rounded-lg text-[9px] font-bold uppercase tracking-widest shadow-sm">Daftar Kawasan</button>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div class="p-6 border-b border-slate-50 dark:border-slate-800">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-bold text-blue-950 dark:text-white uppercase tracking-tight">Database Wilayah Kumuh</h3>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[10px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-200">
                        ...
                        <td class="px-4 py-3">
                            <span class="font-bold text-blue-950 dark:text-white uppercase truncate block text-xs mb-0.5"><?= $item['Kelurahan'] ?></span>
                            <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Kec. <?= $item['Kecamatan'] ?></span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="font-bold text-slate-700 dark:text-slate-200 uppercase tracking-tight"><?= $item['Kawasan'] ?: '-' ?></span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-3 py-1 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg font-bold text-[9px] border border-slate-100 dark:border-slate-700 shadow-sm">
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            ?>
                            <span class="px-3 py-1 rounded-lg border text-[9px] font-bold <?= $colorClass ?>"><?= $skor ?></span>
                        </td>
                        <td class="px-6 py-3 text-center">
                ...
                if (item.WKT && typeof wellknown !== 'undefined') {
                    try {
                        ...
                        if (geojson) {
                            ...
                            const popupContent = `
                                <div class="bg-blue-950 text-white p-3 rounded-t-xl"><p class="text-[7px] font-bold uppercase tracking-widest text-rose-400 mb-1">Kumuh</p><h5 class="text-[11px] font-bold uppercase leading-tight">${item.Kelurahan}</h5></div>
                                <div class="p-3 bg-white dark:bg-slate-900 space-y-2 rounded-b-xl"><div class="flex justify-between items-center"><span class="text-[8px] font-bold text-slate-400 uppercase">Skor</span><span class="px-2 py-0.5 rounded-md bg-rose-50 dark:bg-rose-900/30 text-rose-600 text-[10px] font-bold">${item.skor_kumuh}</span></div><a href="<?= base_url('wilayah-kumuh/detail/') ?>/${item.FID}" class="block w-full py-2.5 bg-blue-950 hover:bg-blue-800 text-white text-center text-[10px] font-black uppercase tracking-[0.2em] rounded-xl shadow-xl transition-all">Detail</a></div>
                            `;
