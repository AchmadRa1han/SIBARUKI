...
<div class="max-w-7xl mx-auto space-y-8 pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center gap-6">
            ...
            <div>
                <h1 class="text-3xl md:text-4xl font-black text-blue-950 dark:text-white uppercase tracking-tighter">Detail PISEW</h1>
                <p class="text-sm text-slate-500 font-bold uppercase tracking-widest mt-1">Tahun Anggaran <?= $item['tahun'] ?></p>
            </div>
        </div>
        <div class="flex items-center gap-3 relative z-10">
            <a href="<?= base_url('pisew/edit/'.$item['id']) ?>" class="px-8 py-4 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-amber-500/20 transition-all active:scale-95 flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden transition-all duration-300">
                <div class="absolute top-0 right-0 p-12 opacity-5 pointer-events-none">
                    <i data-lucide="map" class="w-48 h-48 text-indigo-900 dark:text-white"></i>
                </div>
                
                <h3 class="text-xs font-black text-blue-950 dark:text-white uppercase tracking-[0.3em] mb-10 flex items-center gap-4">
                    <span class="w-10 h-[2px] bg-indigo-600"></span> Identitas & Lokasi Pekerjaan
                </h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-12 relative z-10">
                    <div class="md:col-span-2">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Nama / Jenis Pekerjaan</p>
                    ...
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Lokasi Desa</p>
                        <p class="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-wider"><?= $item['lokasi_desa'] ?></p>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Periode Anggaran</p>
                        <span class="inline-block px-5 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-black uppercase rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                            <?= $item['tahun'] ?>
                        </span>
                    </div>
                    <div class="md:col-span-2 pt-6">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Pihak Pelaksana</p>
                        <div class="p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <p class="text-sm font-bold text-slate-600 dark:text-slate-300 leading-relaxed uppercase"><?= $item['pelaksana'] ?: 'Informasi pelaksana belum tercatat' ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 shadow-sm border border-slate-100 dark:border-slate-800 transition-all duration-300">
                <h3 class="text-xs font-black text-blue-950 dark:text-white uppercase tracking-[0.3em] mb-8 flex items-center gap-4">
                    <span class="w-10 h-[2px] bg-emerald-500"></span> Analisis Pembiayaan
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="bg-emerald-50 dark:bg-emerald-950/20 p-8 rounded-[2rem] border border-emerald-100 dark:border-emerald-900/30 shadow-sm">
                        <p class="text-[10px] font-black text-emerald-600/70 uppercase tracking-widest mb-3">Nilai Kontrak / Pagu</p>
                        <p class="text-3xl font-black text-emerald-600 tracking-tighter italic">Rp <?= number_format($item['anggaran'], 0, ',', '.') ?></p>
                    </div>
                    <div class="bg-indigo-50 dark:bg-indigo-950/20 p-8 rounded-[2rem] border border-indigo-100 dark:border-indigo-900/30 shadow-sm">
                        <p class="text-[10px] font-black text-indigo-600/70 uppercase tracking-widest mb-3">Sumber Pendanaan</p>
                        <p class="text-sm font-black text-indigo-900 dark:text-indigo-400 uppercase tracking-[0.15em]"><?= $item['sumber_dana'] ?: '-' ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-8">
            ...
            <div class="bg-blue-950 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden group">
                ...
                <h4 class="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400 mb-6 flex items-center gap-2">
                    Metadata Inventaris
                </h4>
...
<script>
    function initDetailMap() {
        ...
        const markerIcon = L.divIcon({
            ...
            html: `<div class="w-8 h-8 bg-indigo-600 rounded-full border-4 border-white shadow-2xl flex items-center justify-center animate-bounce-slow"><div class="w-2 h-2 bg-white rounded-full"></div></div>`,
            iconSize: [32, 32],
            iconAnchor: [16, 32]
        });
