    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center gap-4">
            ...
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-blue-950 dark:text-white uppercase tracking-tighter leading-tight">Detail PISEW</h1>
        ...
        <div class="flex items-center gap-2 relative z-10 no-print">
            <button onclick="pisewModal.openEdit(<?= htmlspecialchars(json_encode($item), ENT_QUOTES, 'UTF-8') ?>)" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-amber-500/20 transition-all active:scale-95 flex items-center gap-2">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden transition-all duration-300">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-10 relative z-10">
                    <div class="md:col-span-2">
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Nama / Jenis Pekerjaan</p>
                        <p class="text-xl font-bold text-slate-800 dark:text-slate-100 uppercase tracking-tight"><?= $item['jenis_pekerjaan'] ?></p>
                    </div>
                    <div>
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Wilayah Kecamatan</p>
                        <p class="text-sm font-bold text-indigo-600 uppercase tracking-wider">Kec. <?= $item['kecamatan'] ?></p>
                    </div>
                    <div>
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Lokasi Desa</p>
                        <p class="text-sm font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider"><?= $item['lokasi_desa'] ?></p>
                    </div>
                    <div>
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Periode Anggaran</p>
                        <span class="inline-block px-4 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[10px] font-bold uppercase rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
                            <?= $item['tahun'] ?>
                        </span>
                    </div>
                    <div class="md:col-span-2 pt-4">
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-3">Pihak Pelaksana</p>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-800 transition-all duration-300">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-emerald-50 dark:bg-emerald-950/20 p-6 rounded-xl border border-emerald-100 dark:border-emerald-900/30 shadow-sm">
                        <p class="text-[9px] font-bold text-emerald-600/70 uppercase tracking-widest mb-2">Nilai Kontrak / Pagu</p>
                        <p class="text-3xl font-bold text-emerald-600 tracking-tighter italic">Rp <?= number_format($item['anggaran'], 0, ',', '.') ?></p>
                    </div>
                    <div class="bg-indigo-50 dark:bg-indigo-950/20 p-6 rounded-xl border border-indigo-100 dark:border-indigo-900/30 shadow-sm">
                        <p class="text-[9px] font-bold text-indigo-600/70 uppercase tracking-widest mb-2">Sumber Pendanaan</p>
                        <p class="text-xs font-bold text-indigo-900 dark:text-indigo-400 uppercase tracking-widest"><?= $item['sumber_dana'] ?: '-' ?></p>
                    </div>
                </div>
            </div>
