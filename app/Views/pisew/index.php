...
<div class="space-y-8 pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h1 class="text-3xl md:text-4xl font-black text-blue-950 dark:text-white uppercase tracking-tighter">PISEW</h1>
            <p class="text-slate-500 dark:text-slate-400 font-medium text-sm mt-1">Pengembangan Infrastruktur Sosial Ekonomi Wilayah Kab. Sinjai.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3 relative z-10">
            <div class="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-indigo-100 dark:border-indigo-800/50 shadow-sm">
                <?= number_format($total_kegiatan ?? 0) ?> Kegiatan Terdata
            </div>
            <a href="<?= base_url('pisew/export-excel') ?>" class="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50 hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center gap-3 shadow-sm">
                <i data-lucide="download" class="w-4 h-4"></i> Export Excel
            </a>
            <?php if (has_permission('create_rtlh')): ?>
            <a href="<?= base_url('pisew/create') ?>" class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-600/20 transition-all active:scale-95 flex items-center gap-3 group">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-6">
            <div class="flex items-center gap-4 w-full lg:w-auto">
                <div class="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl w-full md:w-auto">
                    <button class="px-6 py-2.5 bg-white dark:bg-slate-700 text-indigo-600 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">Daftar Kegiatan</button>
                </div>
            </div>
            ...
            <form action="<?= base_url('pisew') ?>" method="get" class="flex flex-col md:flex-row items-center gap-3 w-full lg:w-auto" id="filter-form">
                ...
                <div class="relative w-full md:w-32">
                    <select name="per_page" onchange="submitWithScroll(this)" class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-3 focus:ring-2 focus:ring-indigo-500 cursor-pointer appearance-none">
                ...
                <div class="relative w-full md:w-48">
                    <select name="kecamatan" onchange="submitWithScroll(this)" class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-3 focus:ring-2 focus:ring-indigo-500 cursor-pointer appearance-none">
                        <option value="">Kecamatan</option>
                        <?php foreach($kecamatans as $k): ?>
                            <option value="<?= $k['kecamatan'] ?>" <?= $selected_kecamatan == $k['kecamatan'] ? 'selected' : '' ?>><?= $k['kecamatan'] ?></option>
                        <?php endforeach; ?>
                    </select>
                    <i data-lucide="map-pin" class="w-4 h-4 text-slate-400 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                </div>

                <div class="relative w-full md:w-64">
                    <input type="text" name="search" value="<?= $search ?>" placeholder="Cari desa/kegiatan..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-3 pl-11 focus:ring-2 focus:ring-indigo-500 transition-all">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div id="bulk-action-bar" class="absolute top-0 left-0 right-0 z-50 bg-blue-950 text-white p-5 transform -translate-y-full transition-transform duration-500 flex items-center justify-between px-10">
            <div class="flex items-center gap-5">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest opacity-70 hidden md:block">Aksi massal untuk data PISEW</p>
            </div>
        ...
        <div class="p-8 border-b border-slate-50 dark:border-slate-800">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-lg font-black text-blue-950 dark:text-white uppercase tracking-tight">Data Induk PISEW</h3>
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Manajemen Infrastruktur Wilayah</p>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-4 py-5 w-64 cursor-pointer hover:text-indigo-600 transition-colors" onclick="applySort('jenis_pekerjaan')">
                            <div class="flex items-center gap-2">
                                Jenis Pekerjaan
                                <i data-lucide="arrow-up-down" class="w-3 h-3 opacity-30"></i>
                            </div>
                        </th>
                        <th class="px-4 py-5 w-48">Lokasi & Wilayah</th>
                        <th class="px-4 py-5 w-40 cursor-pointer hover:text-indigo-600 transition-colors" onclick="applySort('anggaran')">
                            <div class="flex items-center gap-2">
                                Anggaran
                                <i data-lucide="arrow-up-down" class="w-3 h-3 opacity-30"></i>
                            </div>
                        </th>
                        <th class="px-4 py-5 w-24 text-center cursor-pointer hover:text-indigo-600 transition-colors" onclick="applySort('tahun')">
                            Tahun
                        </th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[11px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-300">
                        ...
                        <td class="px-4 py-4">
                            <span class="font-black text-blue-950 dark:text-white uppercase truncate block text-sm mb-0.5" title="<?= $item['jenis_pekerjaan'] ?>"><?= $item['jenis_pekerjaan'] ?></span>
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Pelaksana: <?= $item['pelaksana'] ?: '-' ?></span>
                        </td>
                        <td class="px-4 py-4">
                            <div class="flex flex-col gap-0.5">
                                <span class="font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight"><?= $item['lokasi_desa'] ?></span>
                        ...
                        <td class="px-4 py-4">
                            <span class="font-black text-indigo-600 dark:text-indigo-400 tracking-wider">Rp <?= number_format($item['anggaran'], 0, ',', '.') ?></span>
                        </td>
                        <td class="px-4 py-4 text-center">
                            <span class="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-black rounded-lg text-[10px]">
                                <?= $item['tahun'] ?>
                            </span>
                        </td>
                        <td class="px-8 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <button onclick="focusMap(<?= $item['koordinat'] ?>)" class="p-2.5 bg-white dark:bg-slate-800 text-indigo-600 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 hover:bg-indigo-600 hover:text-white transition-all active:scale-95" title="Lihat di Peta"><i data-lucide="map-pin" class="w-4 h-4"></i></button>
                                <?php endif; ?>
                                <a href="<?= base_url('pisew/detail/'.$item['id']) ?>" class="p-2.5 bg-blue-950 dark:bg-indigo-600 text-white rounded-xl shadow-md hover:scale-110 transition-all active:scale-95" title="Detail Data"><i data-lucide="eye" class="w-4 h-4"></i></a>
                                <button onclick="confirmDelete(<?= $item['id'] ?>)" class="p-2.5 bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 rounded-xl hover:bg-rose-600 hover:text-white transition-all active:scale-95" title="Hapus Data"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; else: ?>
            ...
            pisewData.forEach(item => {
                if (item.koordinat) {
                    ...
                    if (coords.length === 2 && !isNaN(coords[0]) && !isNaN(coords[1])) {
                        const marker = L.circleMarker(coords, { radius: 8, fillColor: "#4f46e5", color: "#fff", weight: 2, fillOpacity: 0.8 });
                        marker.bindPopup(`
                            <div class="bg-blue-950 text-white p-4 rounded-t-xl"><p class="text-[8px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-1">Kegiatan PISEW</p><h5 class="text-xs font-black uppercase leading-tight">${item.jenis_pekerjaan}</h5></div>
