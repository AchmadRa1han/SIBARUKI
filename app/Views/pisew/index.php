...
<div class="space-y-6 pb-12 text-slate-900 dark:text-slate-200">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden transition-all duration-300">
        ...
        <div class="relative z-10 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl md:text-3xl font-black text-blue-950 dark:text-white uppercase tracking-tighter">PISEW</h1>
        ...
        <div class="flex flex-wrap items-center gap-2 relative z-10">
            ...
            <a href="<?= base_url('pisew/export-excel') ?>" class="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50 hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center gap-2 shadow-sm">
                <i data-lucide="download" class="w-3.5 h-3.5"></i> Export
            </a>
            <?php if (has_permission('create_rtlh')): ?>
            <a href="<?= base_url('pisew/create') ?>" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-indigo-600/20 transition-all active:scale-95 flex items-center gap-2 group">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-4">
            <div class="flex items-center gap-3 w-full lg:w-auto">
                <div class="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-full md:w-auto">
                    <button class="px-4 py-2 bg-white dark:bg-slate-700 text-indigo-600 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm">Daftar Kegiatan</button>
                </div>
            </div>
            ...
            <form action="<?= base_url('pisew') ?>" method="get" class="flex flex-col md:flex-row items-center gap-2 w-full lg:w-auto" id="filter-form">
                ...
                <div class="relative w-full md:w-64">
                    <input type="text" name="search" value="<?= $search ?>" placeholder="Cari desa/kegiatan..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-[9px] font-black uppercase px-3 py-2 pl-10 focus:ring-2 focus:ring-blue-500 transition-all">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        ...
        <div id="bulk-action-bar" class="absolute top-0 left-0 right-0 z-50 bg-blue-950 text-white p-4 transform -translate-y-full transition-transform duration-500 flex items-center justify-between px-8">
            <div class="flex items-center gap-4">
                <span id="selected-count" class="bg-indigo-600 px-3 py-1 rounded-lg text-[9px] font-black tracking-widest shadow-lg shadow-indigo-600/20">0 TERPILIH</span>
        ...
        <div class="p-6 border-b border-slate-50 dark:border-slate-800">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-black text-blue-950 dark:text-white uppercase tracking-tight">Data Induk PISEW</h3>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 text-[9px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="px-4 py-4 w-64 cursor-pointer hover:text-indigo-600 transition-colors" onclick="applySort('jenis_pekerjaan')">
                            Jenis Pekerjaan
                        </th>
                        <th class="px-4 py-4 w-48">Lokasi & Wilayah</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-[10px]">
                    ...
                    <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all duration-200">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-black rounded-lg text-[9px]">
                                <?= $item['tahun'] ?>
                            </span>
                        </td>
                        <td class="px-6 py-3 text-center">
            ...
            pisewData.forEach(item => {
                if (item.koordinat) {
                    ...
                    if (coords.length === 2 && !isNaN(coords[0]) && !isNaN(coords[1])) {
                        ...
                        marker.bindPopup(`
                            <div class="bg-blue-950 text-white p-3 rounded-t-xl"><p class="text-[7px] font-black uppercase tracking-widest text-indigo-400 mb-1">PISEW</p><h5 class="text-[11px] font-black uppercase leading-tight">${item.jenis_pekerjaan}</h5></div>
                            <div class="p-3 bg-white dark:bg-slate-900 space-y-2 rounded-b-xl"><p class="text-[9px] font-bold text-slate-700">📍 ${item.lokasi_desa}</p><a href="<?= base_url('pisew/detail/') ?>/${item.id}" class="block w-full py-2 bg-blue-950 text-white text-center text-[8px] font-black uppercase tracking-widest rounded-lg transition-all">Detail</a></div>
                        `);
